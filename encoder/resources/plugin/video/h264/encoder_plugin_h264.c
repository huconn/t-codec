...
static int encoder_plugin_h264_Alloc_Memory(ENC_Plugin_H264_T *ptPlugin);
static int encoder_plugin_h264_Free_Memory(ENC_Plugin_H264_T *ptPlugin);
static void encoder_plugin_h264_Set_CLK(ENC_Plugin_H264_T *ptPlugin, TENC_PLUGIN_INIT_T *ptSetInit);

static TENC_PLUGIN_H encoder_plugin_h264_Open(void);
static int encoder_plugin_h264_Close(TENC_PLUGIN_H hPlugin);
static int encoder_plugin_h264_Init(TENC_PLUGIN_H hPlugin, TENC_PLUGIN_INIT_T *ptSetInit);
static int encoder_plugin_h264_Deinit(TENC_PLUGIN_H hPlugin);
static int encoder_plugin_h264_Encode(TENC_PLUGIN_H hPlugin, void *pvEncodeInput, void *pvEncodingResult);
static int encoder_plugin_h264_Set(TENC_PLUGIN_H hPlugin, unsigned int uiId, void *pvStructure);
static int encoder_plugin_h264_Get(TENC_PLUGIN_H hPlugin, unsigned int uiId, void *pvStructure);
static int encoder_plugin_h264_Process_Seq_Header(VEPC_DRIVER_INFO_T *ptDriverInfo, EP_Video_SequenceHeader_Result_T *ptSequenceHeader);
...
static TENC_PLUGIN_H encoder_plugin_h264_Open(void)
{
	void *tmpPtr = TEPV_CALLOC(sizeof(ENC_Plugin_H264_T));
	ENC_Plugin_H264_T *ptPlugin = NULL;
...
static int encoder_plugin_h264_Init(TENC_PLUGIN_H hPlugin, TENC_PLUGIN_INIT_T *ptSetInit)
{
	...
	if (hPlugin != NULL) {
		...
		if (ret != (EP_Video_Error_E)EP_Video_ErrorNone) {
			...
			encoder_plugin_h264_Set_CLK(ptPlugin, ptSetInit);
			(void)encoder_plugin_h264_Alloc_Memory(ptPlugin);
...
static int encoder_plugin_h264_Deinit(TENC_PLUGIN_H hPlugin)
{
	...
	if (hPlugin != NULL) {
		...
		if (ptPlugin->bInited == false) {
			...
			(void)encoder_plugin_h264_Free_Memory(ptPlugin);
			(void)video_encode_plugin_close_drivers(&ptPlugin->tDriverInfo);
		}
	}
	...
	if (hPlugin != NULL) {
		...
		switch (eConfig) {
			case EP_Video_Config_Set_SequenceHeader: {
				...
				iRetVal = encoder_plugin_h264_Process_Seq_Header(&ptPlugin->tDriverInfo, ptSequenceHeader);
				break;
			}
...
TENCODER_PLUGIN_DEFINE(
	TEPV_H264,
	encoder_plugin_h264_Open,
	encoder_plugin_h264_Close,
	encoder_plugin_h264_Init,
	encoder_plugin_h264_Deinit,
	encoder_plugin_h264_Encode,
	encoder_plugin_h264_Set,
	encoder_plugin_h264_Get)