...
static int encoder_plugin_h265_Alloc_Memory(ENC_Plugin_H265_T *ptPlugin);
static int encoder_plugin_h265_Free_Memory(ENC_Plugin_H265_T *ptPlugin);
static void encoder_plugin_h265_Set_CLK(ENC_Plugin_H265_T *ptPlugin, TENC_PLUGIN_INIT_T *ptSetInit);

static TENC_PLUGIN_H encoder_plugin_h265_Open(void);
static int encoder_plugin_h265_Close(TENC_PLUGIN_H hPlugin);
static int encoder_plugin_h265_Init(TENC_PLUGIN_H hPlugin, TENC_PLUGIN_INIT_T *ptSetInit);
static int encoder_plugin_h265_Deinit(TENC_PLUGIN_H hPlugin);
static int encoder_plugin_h265_Encode(TENC_PLUGIN_H hPlugin, void *pvEncodeInput, void *pvEncodingResult);
static int encoder_plugin_h265_Set(TENC_PLUGIN_H hPlugin, unsigned int uiId, void *pvStructure);
static int encoder_plugin_h265_Get(TENC_PLUGIN_H hPlugin, unsigned int uiId, void *pvStructure);
static int encoder_plugin_h265_Process_Seq_Header(VEPC_DRIVER_INFO_T *ptDriverInfo, EP_Video_SequenceHeader_Result_T *ptSequenceHeader);
...
static TENC_PLUGIN_H encoder_plugin_h265_Open(void)
{
	void *tmpPtr = TEPV_CALLOC(sizeof(ENC_Plugin_H265_T));
	ENC_Plugin_H265_T *ptPlugin = NULL;
...
static int encoder_plugin_h265_Init(TENC_PLUGIN_H hPlugin, TENC_PLUGIN_INIT_T *ptSetInit)
{
	...
	if (hPlugin != NULL) {
		...
		ptPlugin->tDriverInfo.uiCodec 			= (unsigned int)VEPK_STD_H265;
		...
		if (ret != (EP_Video_Error_E)EP_Video_ErrorNone) {
			...
			ptPlugin->tDriverInfo.iBitstreamFormat 	= VEPK_STD_H265;
			...
			encoder_plugin_h265_Set_CLK(ptPlugin, ptSetInit);
			(void)encoder_plugin_h265_Alloc_Memory(ptPlugin);
...
static int encoder_plugin_h265_Deinit(TENC_PLUGIN_H hPlugin)
{
	...
	if (hPlugin != NULL) {
		...
		if (ptPlugin->bInited == false) {
			...
			(void)encoder_plugin_h265_Free_Memory(ptPlugin);
			(void)video_encode_plugin_close_drivers(&ptPlugin->tDriverInfo);
		}
	}
...
static int encoder_plugin_h265_Process_Seq_Header(VEPC_DRIVER_INFO_T *ptDriverInfo, EP_Video_SequenceHeader_Result_T *ptSequenceHeader)
{
	...
	if (ptDriverInfo != NULL) {
		if (ptDriverInfo->isSeqHeaderDone == false) {
			...
			iTmpSeqType = (unsigned int)VEPK_HEVC_ENC_CODEOPT_ENC_VPS | (unsigned int)VEPK_HEVC_ENC_CODEOPT_ENC_SPS | (unsigned int)VEPK_HEVC_ENC_CODEOPT_ENC_PPS;
			iSeqType = (int)iTmpSeqType;
			ret = video_encode_plugin_process_seq_header(ptDriverInfo, iSeqType);
	...
	if (hPlugin != NULL) {
		...
		switch (eConfig) {
			case EP_Video_Config_Set_SequenceHeader: {
				...
				iRetVal = encoder_plugin_h265_Process_Seq_Header(&ptPlugin->tDriverInfo, ptSequenceHeader);
				break;
			}
...
TENCODER_PLUGIN_DEFINE(
	TEPV_H265,
	encoder_plugin_h265_Open,
	encoder_plugin_h265_Close,
	encoder_plugin_h265_Init,
	encoder_plugin_h265_Deinit,
	encoder_plugin_h265_Encode,
	encoder_plugin_h265_Set,
	encoder_plugin_h265_Get)