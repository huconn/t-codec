...
#define VP9_MIN_OUTPUT_WIDTH				(DEFAULT_VIDEO_MIN_OUTPUT_WIDTH)
#define VP9_MIN_OUTPUT_HEIGHT				(DEFAULT_VIDEO_MIN_OUTPUT_HEIGHT)
#if defined(SUPPORT_4K_VIDEO)
# define VP9_MAX_OUTPUT_WIDTH   			DEFAULT_4K_VIDEO_MAX_OUTPUT_WIDTH
# define VP9_MAX_OUTPUT_HEIGHT  			DEFAULT_4K_VIDEO_MAX_OUTPUT_HEIGHT
#else
# define VP9_MAX_OUTPUT_WIDTH   			DEFAULT_VIDEO_MAX_OUTPUT_WIDTH
# define VP9_MAX_OUTPUT_HEIGHT  			DEFAULT_VIDEO_MAX_OUTPUT_HEIGHT
#endif
#define VP9_DEFAULT_FRAMERATE				DEFAULT_VIDEO_DEFAULT_FRAMERATE //(30000)
#define VP9_MAX_OUTPUT_REGION				(VP9_MAX_OUTPUT_WIDTH * VP9_MAX_OUTPUT_HEIGHT)
#define VP9_MAX_FRAME_BUFFER_COUNT			(31)
...
static int decoder_plugin_vp9_AllocMemory(DEC_Plugin_VP9_T *ptPlugin);
static int decoder_plugin_vp9_FreeMemory(DEC_Plugin_VP9_T *ptPlugin);
static int decoder_plugin_vp9_DecodeSequenceHeader(DEC_Plugin_VP9_T *ptPlugin, DP_Video_Config_SequenceHeader_T *ptSequenceHeader);

static DEC_PLUGIN_H decoder_plugin_vp9_Open(void);
static int decoder_plugin_vp9_Close(DEC_PLUGIN_H hPlugin);
static int decoder_plugin_vp9_Init(DEC_PLUGIN_H hPlugin, void *pvInitConfig);
static int decoder_plugin_vp9_Deinit(DEC_PLUGIN_H hPlugin);
static int decoder_plugin_vp9_Decode(DEC_PLUGIN_H hPlugin, unsigned int uiStreamLen, unsigned char *pucStream, void *pvDecodingResult);
static int decoder_plugin_vp9_Set(DEC_PLUGIN_H hPlugin, unsigned int uiId, void *pvStructure);
static int decoder_plugin_vp9_Get(DEC_PLUGIN_H hPlugin, unsigned int uiId, void *pvStructure);
...
static int decoder_plugin_vp9_DecodeSequenceHeader(DEC_Plugin_VP9_T *ptPlugin, DP_Video_Config_SequenceHeader_T *ptSequenceHeader)
{
	...
	if (iRetVal == 0) {
		...
		if (iSliceBufSize >= 0) {
			...
			if (video_plugin_cmd(&ptPlugin->tDriverInfo, (VPC_DRIVER_TYPE_E)VPC_DRIVER_TYPE_VPU, VPC_COMMAND_VPU_GET_FREEMEM_SIZE, &iAvailBufSize) < 0) {
				...
				iRetVal = video_plugin_alloc_frame_buffer(&ptPlugin->tDriverInfo, iAvailBufSize, VP9_MAX_FRAME_BUFFER_COUNT);
	...
	if (iRetVal == 0) {
		VPC_RESOLUTION_INFO_T tMaxResInfo = {VP9_MAX_OUTPUT_WIDTH, VP9_MAX_OUTPUT_HEIGHT};
		VPC_RESOLUTION_INFO_T tMinResInfo = {VP9_MIN_OUTPUT_WIDTH, VP9_MIN_OUTPUT_HEIGHT};
...
static DEC_PLUGIN_H decoder_plugin_vp9_Open(void)
{
	void *tmpPtr = DPV_CALLOC(sizeof(DEC_Plugin_VP9_T));
	DEC_Plugin_VP9_T *ptPlugin = NULL;
...
static int decoder_plugin_vp9_Init(DEC_PLUGIN_H hPlugin, void *pvInitConfig)
{
	...
	ptPlugin->tDriverInfo.uiCodec 		= VPK_STD_VP9;
	...
	if (ret != (DP_Video_Error_E)DP_Video_ErrorNone) {
		...
		ptDriverInfo->iBitstreamFormat 		= VPK_STD_VP9;
		ptDriverInfo->iPicWidth				= VP9_MAX_OUTPUT_WIDTH;
		ptDriverInfo->iPicHeight			= VP9_MAX_OUTPUT_HEIGHT;
		ptDriverInfo->bEnableUserData		= 0;
		ptDriverInfo->bCbCrInterleaveMode 	= 1;
		ptDriverInfo->iFilePlayEnable 		= 1;
		ptDriverInfo->iFrameRate 			= VP9_DEFAULT_FRAMERATE;
		ptDriverInfo->iBitRateMbps 			= 10;
		ptDriverInfo->iExtFunction 			= 0;

		video_plugin_set_init_info(ptDriverInfo);

		(void)decoder_plugin_vp9_AllocMemory(ptPlugin);
...
static int decoder_plugin_vp9_Deinit(DEC_PLUGIN_H hPlugin)
{
	...
	if (ptPlugin->bInited == false) {
		...
		(void)decoder_plugin_vp9_FreeMemory(ptPlugin);
	...
	switch (eConfig) {
		...
		case DP_Video_Config_Set_VPU_Clock: {
			...
			if ((ptPlugin->tDriverInfo.iFrameRate > 0) && (ptPlugin->tDriverInfo.iBitRateMbps > 0)) {
				...
				tContentInfo.width 		= VP9_MAX_OUTPUT_WIDTH;
				tContentInfo.height	 	= VP9_MAX_OUTPUT_HEIGHT;
		...
		case DP_Video_Config_Set_SequenceHeader: {
			...
			iRetVal = decoder_plugin_vp9_DecodeSequenceHeader(ptPlugin, ptSequenceHeader);
			break;
		}
...
DECODER_PLUGIN_DEFINE(
	DPV_MPG2,
	decoder_plugin_vp9_Open,
	decoder_plugin_vp9_Close,
	decoder_plugin_vp9_Init,
	decoder_plugin_vp9_Deinit,
	decoder_plugin_vp9_Decode,
	decoder_plugin_vp9_Set,
	decoder_plugin_vp9_Get)