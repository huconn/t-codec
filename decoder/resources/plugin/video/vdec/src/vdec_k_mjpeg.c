...
static int jpu_cmd_process(int cmd, unsigned long* args, tc_vdec_ *pVdec)
{
  int ret = 0;
  int success = 0;
  ...
  if(ret < 0)
  {
    ...
    else
    {
      ...
      if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
        ...
        LOGE("JPU[%d] ioctl err[%s] : cmd = 0x%x", pInst->vdec_instance_index, errmsg, cmd);
      }
    }
  }
...
Retry:
  while (retry_cnt > 0) {
    ...
    if (ret < 0) {
      ...
      if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
        ...
        LOGE("JPU[%d]-retry(%d:cmd(%d)) poll error '%s'", pInst->vdec_instance_index, retry_cnt, cmd, errmsg);
      }
      retry_cnt--;
      continue;
    }else if (ret == 0) {
  ...
  switch(cmd)
  {
    case V_DEC_INIT:
    {
      const JDEC_INIT_t* init_info = (JDEC_INIT_t*)CONV_PTR(args);
      if(ioctl(pInst->dec_fd, V_DEC_INIT_RESULT, args) < 0){
        char errmsg[STRERR_BUFFERSIZE];
        if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
          LOGE("can't get errmsg from strerror_r");
        } else {
          LOGE("JPU[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_DEC_INIT_RESULT, errmsg);
        }
      }
    ...
    {
      const JDEC_SEQ_HEADER_t* seq_info = (JDEC_SEQ_HEADER_t *)CONV_PTR(args);
    ...
    case V_DEC_DECODE:
    {
      const JPU_DECODE_t* decoded_info = (JPU_DECODE_t *)CONV_PTR(args);
      ...
      if(ioctl(pInst->dec_fd, V_DEC_DECODE_RESULT, args) < 0){
        ...
        if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
          ...
          LOGE("JPU[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_DEC_DECODE_RESULT, errmsg);
        }
      }
    ...
    {
      const JPU_GET_VERSION_t* p_param = (JPU_GET_VERSION_t*)CONV_PTR(args);
      if(ioctl(pInst->dec_fd, V_GET_VPU_VERSION_RESULT, args) < 0){
        char errmsg[STRERR_BUFFERSIZE];
        if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
          LOGE("can't get errmsg from strerror_r");
        } else {
          LOGE("JPU[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_GET_VPU_VERSION_RESULT, errmsg);
        }
      }
      ret = p_param->result;
    }
    ...
    default:
      ...
      if(ioctl(pInst->dec_fd, V_DEC_GENERAL_RESULT, &ret) < 0){
        ...
        if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
          ...
          LOGE("JPU[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_DEC_GENERAL_RESULT, errmsg);
        }
      }
    }
    break;
...
int32_t vdec_mjpeg_jpu(int32_t iOpCode, unsigned long* pHandle, unsigned long* pParam1, unsigned long* pParam2, unsigned long* pParam3)     // For TCC892XS / TCC893XS, use JPU as a MJPEG H/W decoder
{
  ...
  if( iOpCode == VDEC_INIT )
  {
    ...
    if(p_init_param->m_iBitstreamBufSize > LARGE_STREAM_BUF_SIZE){
      pInst->gsBitstreamBufSize = ALIGN_BUFF( p_init_param->m_iBitstreamBufSize, 64*1024 );
    } else {
    ...
    {
      LOGE("[JPU-%d] bitstream_buf_addr[%s] malloc() failed \r", pInst->vdec_instance_index,(pInst->gsBitstreamBufAddr[PA] == 0U) ? ("PA") : ("VA"));
    ...
    pInst->gsJpuDecInit_Info.gsJpuDecInit.m_iCbCrInterleaveMode  = tcomx_u32_to_s32(p_init_param->m_bCbCrInterleaveMode);
    pInst->gsJpuDecInit_Info.gsJpuDecInit.m_uiDecOptFlags = 0u;
    ...
    ret = jpu_cmd_process(V_DEC_INIT, (unsigned long *)CONV_PTR(&pInst->gsJpuDecInit_Info), pInst);
    ...
    if( (pInst->extFunction & (TC_EXT_FUNC_MEM_PROTECTION_WITH_INSTREAM | TC_EXT_FUNC_MEM_PROTECTION_ONLY)) == 0x0u )
    {
      if (pInst->gsBitstreamBufSize > 100) {
        ...
        pInst->gsJpuDecVersion.pszVersion = &p_version[pInst->gsBitstreamBufSize - 100];
        pInst->gsJpuDecVersion.pszBuildData = &p_version[pInst->gsBitstreamBufSize - 50];

        ret = jpu_cmd_process(V_GET_VPU_VERSION, (unsigned long *)CONV_PTR(&pInst->gsJpuDecVersion), pInst);
  ...
  else if( iOpCode == VDEC_DEC_SEQ_HEADER )
  {
    ...
    else
    {
      ...
      DSTATUS("[JPU-%d] copyed input-stream = %d (from 0x%p)", pInst->vdec_instance_index, seq_stream_size, (unsigned long *)p_input_param->m_pInp[VA]);
    ...
    pInst->gsJpuDecSeqHeader_Info.stream_size = tcomx_s32_to_u32(seq_stream_size);
    ret = jpu_cmd_process(V_DEC_SEQ_HEADER, (unsigned long *)CONV_PTR(&pInst->gsJpuDecSeqHeader_Info), pInst);
    if( ret != JPG_RET_SUCCESS )
    {
    ...
    //------------------------------------------------------------
    // scale factor set
    pInst->gsJpuDecBuffer_Info.gsJpuDecBuffer.m_iJPGScaleRatio = tcomx_u32_to_s32(pInst->gsUserInfo.jpg_ScaleRatio);        //!< JPEG Scaling Ratio

    minFrameBuffSize = pInst->gsJpuDecSeqHeader_Info.gsJpuDecInitialInfo.m_iMinFrameBufferSize[pInst->gsUserInfo.jpg_ScaleRatio];
    ...
    max_count = vdec_sys_remain_memory_size(pInst) / minFrameBuffSize;
    ...
    pInst->gsFrameBufSize = vdec_sys_malloc_dual_addr(pInst->gsFrameBufAddr, pInst->gsFrameBufSize,  4096u, BUFFER_FRAMEBUFFER, pInst);
    if( (pInst->gsFrameBufAddr[PA] == 0u) || (pInst->gsFrameBufAddr[VA] == 0u))
    {
      LOGE("[JPU-%d] pInst->gsFrameBufAddr[%s] malloc() failed \r", pInst->vdec_instance_index,(pInst->gsFrameBufAddr[PA] == 0U) ? ("PA") : ("VA"));
      ret = -(VPU_NOT_ENOUGH_MEM);
      goto vdec_mjpeg_jpu_end;
    }
    ...
    ret = jpu_cmd_process(V_DEC_REG_FRAME_BUFFER, (unsigned long *)CONV_PTR(&pInst->gsJpuDecBuffer_Info), pInst);
    if( ret != JPG_RET_SUCCESS )
    {
      ...
      LOGI( "[JPU-%d] VDEC_DEC_SEQ_HEADER - Success mem_free = 0x%x ", pInst->vdec_instance_index, vdec_sys_final_free_mem(pInst) );
  ...
  else if( iOpCode == VDEC_DECODE )
  {
    ...
    else
    {
...
#endif
      {
        ...
        {
          tcomx_memcpy( pInst->gsBitstreamBufAddr[VA], p_input_param->m_pInp[VA], pInst->gsJpuDecInOut_Info.gsJpuDecInput.m_iBitstreamDataSize);
    ...
    if( (pInst->extFunction & (TC_EXT_FUNC_MEM_PROTECTION_WITH_INSTREAM | TC_EXT_FUNC_MEM_PROTECTION_ONLY)) == 0x0u )
    {
      ...
      int32_t len = pInst->gsJpuDecInOut_Info.gsJpuDecInput.m_iBitstreamDataSize;
    ...
    ret = jpu_cmd_process(V_DEC_DECODE, (unsigned long *)CONV_PTR(&pInst->gsJpuDecInOut_Info), pInst);
    ...
    {
      {
        ...
        DSTATUS("[JPU-%d] yuv(0x%lx) stream", pInst->vdec_instance_index, (unsigned long)p_output_param->m_pDispOut[VA][COMP_Y]);
      }
  ...
  else if( iOpCode == VDEC_CLOSE )
  {
    ...
    ret = jpu_cmd_process(V_DEC_CLOSE, (unsigned long *)CONV_PTR(&pInst->gsJpuDecInOut_Info), pInst);
    if(ret != JPG_RET_SUCCESS)
    {
