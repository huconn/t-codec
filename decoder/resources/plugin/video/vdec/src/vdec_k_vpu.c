...
static int32_t vdec_cmd_process(int32_t cmd, unsigned long* args, tc_vdec_ *pVdec)
{
  int32_t ret;
  ...
  if(ret < 0)
  {
    ...
    else
    {
      ...
      if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
        ...
        LOGE("VDEC[%d] ioctl err[%s] : cmd = 0x%x", pInst->vdec_instance_index, errmsg, cmd);
      }
    }
  }
...
Retry:
  while (retry_cnt > 0) {
    ...
    if (ret < 0) {
      ...
      if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
        ...
        LOGE("VDEC[%d] -retry(%d:cmd(%d)) poll error '%s'", pInst->vdec_instance_index, retry_cnt, cmd, errmsg);
      }
      retry_cnt--;
      continue;
    }else if (ret == 0) {
  ...
  switch(cmd){
    case V_DEC_INIT:
    {
      const VDEC_INIT_t* init_info = (VDEC_INIT_t *)CONV_PTR(args);
      ...
      if(ioctl(pInst->dec_fd, V_DEC_INIT_RESULT, args) < 0){
        ...
        if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
          ...
          LOGE("VDEC[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_DEC_INIT_RESULT, errmsg);
        }
      }
    ...
    case V_DEC_SEQ_HEADER:
    {
      ...
      const VDEC_DECODE_t* seq_info = (VDEC_DECODE_t *)CONV_PTR(args);
#else
      const VDEC_SEQ_HEADER_t* seq_info = (VDEC_SEQ_HEADER_t *)CONV_PTR(args);
#endif
      if(ioctl(pInst->dec_fd, V_DEC_SEQ_HEADER_RESULT, args) < 0){
        ...
        if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
          ...
          LOGE("VDEC[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_DEC_SEQ_HEADER_RESULT, errmsg);
        }
      }
      ret = seq_info->result;
  #ifdef ERROR_TEST
    ...
    case V_DEC_DECODE:
    {
      const VDEC_DECODE_t* decoded_info = (VDEC_DECODE_t *)CONV_PTR(args);
      ...
      if(ioctl(pInst->dec_fd, V_DEC_DECODE_RESULT, args) < 0){
        ...
        if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
          ...
          LOGE("VDEC[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_DEC_DECODE_RESULT, errmsg);
        }
      }
    ...
    case V_DEC_FLUSH_OUTPUT:
    {
      ...
      if(ioctl(pInst->dec_fd, V_DEC_FLUSH_OUTPUT_RESULT, args) < 0){
        ...
        if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
          ...
          LOGE("VDEC[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_DEC_FLUSH_OUTPUT_RESULT, errmsg);
        }
      }
      ret = decoded_info->result;
    }
    ...
    {
      const VDEC_RINGBUF_GETINFO_t* p_param = (VDEC_RINGBUF_GETINFO_t *)CONV_PTR(args);
      if(ioctl(pInst->dec_fd, V_GET_RING_BUFFER_STATUS_RESULT, args) < 0){
        char errmsg[STRERR_BUFFERSIZE];
        if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
          LOGE("can't get errmsg from strerror_r");
        } else {
          LOGE("VDEC[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_GET_RING_BUFFER_STATUS_RESULT, errmsg);
        }
      }
      ret = p_param->result;
    }
    ...
    {
      const VDEC_RINGBUF_SETBUF_t* p_param = (VDEC_RINGBUF_SETBUF_t *)CONV_PTR(args);
      if(ioctl(pInst->dec_fd, V_FILL_RING_BUFFER_AUTO_RESULT, args) < 0){
        char errmsg[STRERR_BUFFERSIZE];
        if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
          LOGE("can't get errmsg from strerror_r");
        } else {
          LOGE("VDEC[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_FILL_RING_BUFFER_AUTO_RESULT, errmsg);
        }
      }
      ret = p_param->result;
    }
    ...
    {
      const VDEC_RINGBUF_SETBUF_PTRONLY_t* p_param = (VDEC_RINGBUF_SETBUF_PTRONLY_t *)CONV_PTR(args);
      if(ioctl(pInst->dec_fd, V_DEC_UPDATE_RINGBUF_WP_RESULT, args) < 0){
        char errmsg[STRERR_BUFFERSIZE];
        if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
          LOGE("can't get errmsg from strerror_r");
        } else {
          LOGE("VDEC[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_DEC_UPDATE_RINGBUF_WP_RESULT, errmsg);
        }
      }
      ret = p_param->result;
    }
    ...
    {
      const VDEC_SEQ_HEADER_t* p_param = (VDEC_SEQ_HEADER_t *)CONV_PTR(args);
      if(ioctl(pInst->dec_fd, V_GET_INITIAL_INFO_FOR_STREAMING_MODE_ONLY_RESULT, args) < 0){
        char errmsg[STRERR_BUFFERSIZE];
        if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
          LOGE("can't get errmsg from strerror_r");
        } else {
          LOGE("VDEC[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_GET_INITIAL_INFO_FOR_STREAMING_MODE_ONLY_RESULT, errmsg);
        }
      }
      ret = p_param->result;
    }
    ...
    {
      const VDEC_GET_VERSION_t* p_param = (VDEC_GET_VERSION_t *)CONV_PTR(args);
      if(ioctl(pInst->dec_fd, V_GET_VPU_VERSION_RESULT, args) < 0){
        char errmsg[STRERR_BUFFERSIZE];
        if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
          LOGE("can't get errmsg from strerror_r");
        } else {
          LOGE("VDEC[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_GET_VPU_VERSION_RESULT, errmsg);
        }
      }
      ret = p_param->result;
    }
    ...
    default:
    {
      if(ioctl(pInst->dec_fd, V_DEC_GENERAL_RESULT, &ret) < 0){
        ...
        if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
          ...
          LOGE("VDEC[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_DEC_GENERAL_RESULT, errmsg);
        }
      }
    }
    break;
...
static int vpu_dec_ready( dec_init_t* psVDecInit, tc_vdec_ *pVdec)
{
  tc_vdec_ * pInst = (tc_vdec_ *)pVdec;
  ...
  //------------------------------------------------------------
  pInst->gsBitstreamBufSize = vdec_sys_malloc_dual_addr(pInst->gsBitstreamBufAddr,
                              ((psVDecInit->m_iBitstreamBufSize > LARGE_STREAM_BUF_SIZE) ? (pInst->gsBitstreamBufSize) : (LARGE_STREAM_BUF_SIZE)),
                              ((psVDecInit->m_iBitstreamBufSize > LARGE_STREAM_BUF_SIZE) ? (64*1024u) : (DPV_ALIGN_LEN)),
                              BUFFER_STREAM, pInst);

  if( (pInst->gsBitstreamBufAddr[PA] == 0u) || (pInst->gsBitstreamBufAddr[VA] == 0u)){
    DPRINTF( "[VDEC-%d] bitstream_buf_addr[%s] malloc() failed ", pInst->vdec_instance_index, (pInst->gsBitstreamBufAddr[PA] == 0U) ? ("PA") : ("VA"));
    ret = -(VPU_NOT_ENOUGH_MEM);
    goto vpu_dec_ready_end;
  }
...
#if 1
    if( (pInst->extFunction & (TC_EXT_FUNC_MEM_PROTECTION_WITH_INSTREAM | TC_EXT_FUNC_MEM_PROTECTION_ONLY)) == 0x0u )
    {
      tcomx_zero_memset( pInst->gsBitstreamBufAddr[VA], 0x00 , pInst->gsBitstreamBufSize);
    }
#endif
  ...
  {
    temp_uiSize = (uint32_t)PS_SAVE_SIZE;
    pInst->gsSpsPpsSize = tcomx_u32_to_s32(ALIGN_BUFF( temp_uiSize, DPV_ALIGN_LEN ));
    pInst->gsSpsPpsAddr = (codec_addr_t)vdec_sys_malloc_physical_addr( NULL, pInst->gsSpsPpsSize, BUFFER_PS, pInst );
    if( pInst->gsSpsPpsAddr == 0u )
    {
      DPRINTF( "[VDEC-%d] sps_pps_buf_addr malloc() failed ", pInst->vdec_instance_index);
      ret = -(VPU_NOT_ENOUGH_MEM);
      goto vpu_dec_ready_end;
    }
    DSTATUS("[VDEC-%d] sps_pps_buf_addr = 0x%lx, 0x%x ", pInst->vdec_instance_index, (codec_addr_t)pInst->gsSpsPpsAddr, pInst->gsSpsPpsSize );
  ...
  if(pInst->gsVpuDecInit_Info.gsVpuDecInit.m_iFilePlayEnable == 0)
  {
    ...
    pInst->gsIntermediateBufSize = ALIGN_BUFF( pInst->gsIntermediateBufSize, DPV_ALIGN_LEN );
    pInst->gsIntermediateBufAddr[PA] = (codec_addr_t)vdec_sys_malloc_physical_addr(&pInst->gsIntermediateBufAddr[K_VA], pInst->gsIntermediateBufSize, BUFFER_STREAM, pInst );
    ...
    pInst->gsIntermediateBufAddr[VA] = (codec_addr_t)vdec_sys_malloc_virtual_addr( (unsigned long *)pInst->gsIntermediateBufAddr[PA], pInst->gsIntermediateBufSize, pInst );
...
//  gsMaxBitstreamSize = ALIGN_BUFF(gsMaxBitstreamSize, (4 * 1024));
...
//  }
  ...
  {
    pInst->gsUserdataBufSize = vdec_sys_malloc_dual_addr(pInst->gsUserdataBufAddr, 50 * 1024, DPV_ALIGN_LEN, BUFFER_USERDATA, pInst );
    if( (pInst->gsUserdataBufAddr[PA] == 0UL) || (pInst->gsUserdataBufAddr[VA] == 0UL))
    {
      DPRINTF( "[VDEC-%d:Err%d] pInst->gsUserdataBufAddr[%s] alloc failed ", pInst->vdec_instance_index, -1, (pInst->gsUserdataBufAddr[PA] == 0U) ? ("PA") : ("VA"));
      ret = -(VPU_NOT_ENOUGH_MEM);
      goto vpu_dec_ready_end;
    }
    ...
    DSTATUS("[VDEC-%d] pInst->gsUserdataBufAddr[VA] = 0x%lx, 0x%x ", pInst->vdec_instance_index, (codec_addr_t)pInst->gsUserdataBufAddr[VA], pInst->gsUserdataBufSize );
  }

  //------------------------------------------------------------
  // [x] code buffer, work buffer and parameter buffer for VPU
  //------------------------------------------------------------
  pInst->gsBitWorkBufSize = vdec_sys_malloc_dual_addr( pInst->gsBitWorkBufAddr, WORK_CODE_PARA_BUF_SIZE, DPV_ALIGN_LEN, BUFFER_WORK, pInst );
  if( (pInst->gsBitWorkBufAddr[PA] == 0U) || (pInst->gsBitWorkBufAddr[VA] == 0U))
  {
    DPRINTF( "[VDEC-%d] bit_work_buf_addr[%s] malloc() failed ", pInst->vdec_instance_index, (pInst->gsBitWorkBufAddr[PA] == 0U) ? ("PA") : ("VA"));
    ret = -(VPU_NOT_ENOUGH_MEM);
    goto vpu_dec_ready_end;
  }
  ...
  if( (psVDecInit->m_uiDecOptFlags&(uint32_t)M4V_DEBLK_ENABLE) != 0U){
    DSTATUS( "[VDEC-%d] MPEG-4 Deblocking ON" , pInst->vdec_instance_index);
  }
  if( (psVDecInit->m_uiDecOptFlags&(uint32_t)M4V_GMC_FRAME_SKIP) != 0U){
...
static int vpu_dec_seq_header( int iSize, int iIsThumbnail, tc_vdec_ *pVdec )
{
  int ret = 0;
  uint32_t temp_uiSize;
  tc_vdec_ * pInst = (tc_vdec_ *)pVdec;
...
#if REMOVE_SECURE_COPY
  vdec_info_t * pVdecInfo = (vdec_info_t *)CONV_PTR(&pInst->gsVpuDecInOut_Info.gsVpuDecInitialInfo);
  unsigned long * pSeq_info = (unsigned long *)CONV_PTR(&pInst->gsVpuDecInOut_Info);
#else
  vdec_info_t * pVdecInfo = (vdec_info_t *)CONV_PTR(&pInst->gsVpuDecSeqHeader_Info.gsVpuDecInitialInfo);
  unsigned long * pSeq_info = (unsigned long *)CONV_PTR(&pInst->gsVpuDecSeqHeader_Info);
  pInst->gsVpuDecSeqHeader_Info.stream_size = tcomx_s32_to_u32(iSize);
#endif
  ...
  ret = vdec_cmd_process(V_DEC_SEQ_HEADER, pSeq_info, pInst);
  if( ret != RETCODE_SUCCESS )
  {
    DPRINTF( "[VDEC-%d] VPU_DEC_SEQ_HEADER failed Error code is 0x%x. ErrorReason is %d",
        pInst->vdec_instance_index, ret, pVdecInfo->m_iReportErrorReason);
    ...
    pInst->gsCommDecInfo.m_iErrCode = pVdecInfo->m_iReportErrorReason; // mm008
    ret = -ret;
    goto vpu_dec_seq_header_end;
  }

  print_dec_initial_info( &pInst->gsVpuDecInit_Info.gsVpuDecInit, pVdecInfo, pInst );
  ...
  if( (pInst->gsVpuDecInit_Info.gsVpuDecInit.m_iBitstreamFormat == STD_AVC) || (pInst->gsVpuDecInit_Info.gsVpuDecInit.m_iBitstreamFormat == STD_MVC) ){
    pInst->mRealPicWidth = pVdecInfo->m_iPicWidth
                         - pVdecInfo->m_iAvcPicCrop.m_iCropLeft
                         - pVdecInfo->m_iAvcPicCrop.m_iCropRight;
    pInst->mRealPicHeight = pVdecInfo->m_iPicHeight
                          - pVdecInfo->m_iAvcPicCrop.m_iCropBottom
                          - pVdecInfo->m_iAvcPicCrop.m_iCropTop;
  }
  else{
    pInst->mRealPicWidth = pVdecInfo->m_iPicWidth;
    pInst->mRealPicHeight = pVdecInfo->m_iPicHeight;
  ...
  if( (pInst->extFunction & TC_EXT_FUNC_MAX_FRAMEBUFFER) != 0x0u )
  {
    pVdecInfo->m_iPicWidth = VPU_FRAMEBUFFER_MAX_WIDTH;//AVAILABLE_MAX_WIDTH;
    pVdecInfo->m_iPicHeight = VPU_FRAMEBUFFER_MAX_HEIGHT;//AVAILABLE_MAX_HEIGHT;
    LOGI("[VDEC-%d]Set seq framebuffer into 1080p (<- %d x %d)", pInst->vdec_instance_index, pInst->mRealPicWidth, pInst->mRealPicHeight);
  }
#endif

  set_dec_common_info(pVdecInfo->m_iPicWidth,
      pVdecInfo->m_iPicHeight,
     &pVdecInfo->m_iAvcPicCrop,
      pVdecInfo->m_iInterlace,
      0, pInst );
  ...
  if( (pInst->gsVpuDecInit_Info.gsVpuDecInit.m_iBitstreamFormat == STD_AVC) || (pInst->gsVpuDecInit_Info.gsVpuDecInit.m_iBitstreamFormat == STD_MVC) )
  {
    temp_uiSize = (uint32_t)SLICE_SAVE_SIZE;
    pInst->gsSliceBufSize = tcomx_u32_to_s32(ALIGN_BUFF( temp_uiSize, DPV_ALIGN_LEN ));
    pInst->gsSliceBufAddr = (codec_addr_t)vdec_sys_malloc_physical_addr( NULL, pInst->gsSliceBufSize, BUFFER_SLICE, pInst );
    if( pInst->gsSliceBufAddr == 0u )
    {
      DPRINTF( "[VDEC-%d] slice_buf_addr malloc() failed ", pInst->vdec_instance_index);
      ret = -(VPU_NOT_ENOUGH_MEM);
      goto vpu_dec_seq_header_end;
    }
    DSTATUS("[VDEC-%d] slice_buf_addr = 0x%lx, 0x%x ", pInst->vdec_instance_index, (codec_addr_t)pInst->gsSliceBufAddr, pInst->gsSliceBufSize );

    pInst->gsVpuDecBuffer_Info.gsVpuDecBuffer.m_AvcSliceSaveBufferAddr  = tcomx_ulong_to_u32(pInst->gsSliceBufAddr);
  ...
  {
    temp_uiSize = (uint32_t)(17*4*((pVdecInfo->m_iPicWidth * pVdecInfo->m_iPicHeight)>>8));
    pInst->gsMbSaveSize = tcomx_u32_to_s32(ALIGN_BUFF( temp_uiSize, 4*1024 ));
    pInst->gsMbSaveAddr = (codec_addr_t)vdec_sys_malloc_physical_addr( NULL, pInst->gsMbSaveSize, BUFFER_ELSE, pInst );
    if( pInst->gsMbSaveAddr == 0u )
    {
      DPRINTF( "[VDEC-%d] MbSaveAddr malloc() failed ", pInst->vdec_instance_index);
      ret = -(VPU_NOT_ENOUGH_MEM);
      goto vpu_dec_seq_header_end;
    }
    DSTATUS("[VDEC-%d] MbSaveAddr = 0x%lx, 0x%x ", pInst->vdec_instance_index, (codec_addr_t)pInst->gsMbSaveAddr, pInst->gsMbSaveSize );

    pInst->gsVpuDecBuffer_Info.gsVpuDecBuffer.m_Vp8MbDataSaveBufferAddr = tcomx_u64_to_u32(pInst->gsMbSaveAddr);
  ...
  //------------------------------------------------------------
  pInst->gsVpuDecBuffer_Info.gsVpuDecBuffer.m_iFrameBufferCount = pVdecInfo->m_iMinFrameBufferCount;
  LOGD( "[VDEC-%d] FrameBufDelay %d, MinFrameBufferCount %d", pInst->vdec_instance_index,
       pVdecInfo->m_iFrameBufDelay, pVdecInfo->m_iMinFrameBufferCount);

  {
    ...
    if(iIsThumbnail == 0){
      pInst->gsVpuDecBuffer_Info.gsVpuDecBuffer.m_iFrameBufferCount =
          pVdecInfo->m_iMinFrameBufferCount + pInst->gsAdditionalFrameCount;
    }
    max_count = vdec_sys_remain_memory_size(pInst) / pVdecInfo->m_iMinFrameBufferSize;
    ...
    if(iIsThumbnail != 0)
    {
      if(pInst->gsVpuDecBuffer_Info.gsVpuDecBuffer.m_iFrameBufferCount < (pVdecInfo->m_iMinFrameBufferCount))
      {
        tcc_printf( "[VDEC-%d] [line:%d] Not enough memory for VPU frame buffer, Available[%d], Min[%d], Need[%d], min_size = %d",
                      ...
                      pVdecInfo->m_iMinFrameBufferCount,
                      pInst->gsVpuDecBuffer_Info.gsVpuDecBuffer.m_iFrameBufferCount,
                      pVdecInfo->m_iMinFrameBufferSize);
        ret = -(VPU_NOT_ENOUGH_MEM);
        goto vpu_dec_seq_header_end;
      }
    }
    ...
    {
      if(pInst->gsVpuDecBuffer_Info.gsVpuDecBuffer.m_iFrameBufferCount < (pVdecInfo->m_iMinFrameBufferCount/*+pInst->gsAdditionalFrameCount*/))
      {
        tcc_printf( "[VDEC-%d] [line:%d] Not enough memory for VPU frame buffer, Available[%d], Min[%d], Need[%d], min_size = %d",
                      pInst->vdec_instance_index, __LINE__, max_count,
                      pVdecInfo->m_iMinFrameBufferCount,
  ...
  pInst->gsFrameBufSize = pInst->gsVpuDecBuffer_Info.gsVpuDecBuffer.m_iFrameBufferCount * pVdecInfo->m_iMinFrameBufferSize;
  ...
  {
    DPRINTF("[VDEC-%d] MinFrameBufferSize %d bytes, FrameBufSize %d bytes",
          pInst->vdec_instance_index, pVdecInfo->m_iMinFrameBufferSize, pInst->gsFrameBufSize);
    DPRINTF( "[VDEC-%d][line:%d] frame_buf_addr[%s] malloc() failed ", pInst->vdec_instance_index, __LINE__, (pInst->gsFrameBufAddr[PA] == 0U) ? ("PA") : ("VA"));
    ret = -(VPU_NOT_ENOUGH_MEM);
    goto vpu_dec_seq_header_end;
  }
  ...
  DSTATUS("[VDEC-%d] frame_buf_addr[VA] = 0x%lx, frame_buf_addr[K_VA] = 0x%lx ", pInst->vdec_instance_index,
  ...
  ret = vdec_cmd_process(V_DEC_REG_FRAME_BUFFER, (unsigned long *)CONV_PTR(&pInst->gsVpuDecBuffer_Info), pInst);
  if( ret != RETCODE_SUCCESS )
  {
...
vdec_vpu( int32_t iOpCode, unsigned long* pHandle, unsigned long* pParam1, unsigned long* pParam2, unsigned long* pParam3 )
{
  ...
  switch(iOpCode)
  {
    case VDEC_INIT:
    {
      ...
      const vdec_user_info_t* p_init_user_param = (vdec_user_info_t*)CONV_PTR(pParam2);
      ...
      pInst->gsVpuDecInit_Info.gsVpuDecInit.m_RegBaseVirtualAddr = 0u;
      ...
      if( (pInst->extFunction & TC_EXT_FUNC_MAX_FRAMEBUFFER) != 0x0u )
      {
        pInst->gsVpuDecInit_Info.gsVpuDecInit.m_uiDecOptFlags |= (1u<<16);
        DSTATUS("[VDEC-%d]Set framebuffer into 1080p", pInst->vdec_instance_index);
      }
  #endif

      pInst->gsVpuDecInit_Info.gsVpuDecInit.m_iFilePlayEnable   = tcomx_u32_to_s32(p_init_param->m_bFilePlayEnable);
      ...
      if( (pInst->extFunction & TC_EXT_FUNC_NO_BUFFER_DELAY) != 0x0u )
      {
        LOGI("[VDEC_K] : No BufferDelay Mode....");
        pInst->gsVpuDecInit_Info.gsVpuDecInit.m_uiDecOptFlags |= (1u<<2);
      }

  #if REMOVE_SECURE_COPY
      pInst->gsVpuDecInit_Info.gsVpuDecInit.m_uiDecOptFlags |= (1u<<26);
  #endif
      ...
      DSTATUS("[VDEC-%d]optFlag 0x%x, avcBuff: 0x%p- %d, Userdata(%d), VCache: %d, Inter: %d, PlayEn: %d, MaxRes: %d", pInst->vdec_instance_index,
            pInst->gsVpuDecInit_Info.gsVpuDecInit.m_uiDecOptFlags,
            pInst->gsVpuDecInit_Info.gsVpuDecInit.m_pSpsPpsSaveBuffer,
      ...
      ret = vdec_cmd_process(V_DEC_INIT, (unsigned long *)CONV_PTR(&pInst->gsVpuDecInit_Info), pInst);
      if( ret != RETCODE_SUCCESS )
      {
      ...
      if( (pInst->extFunction & (TC_EXT_FUNC_MEM_PROTECTION_WITH_INSTREAM | TC_EXT_FUNC_MEM_PROTECTION_ONLY)) == 0x0u )
      {
        if (pInst->gsBitstreamBufSize > 100) {
          ...
          pInst->gsVpuDecVersion.pszVersion = &p_version[pInst->gsBitstreamBufSize - 100];
          pInst->gsVpuDecVersion.pszBuildData = &p_version[pInst->gsBitstreamBufSize - 50];

          ret = vdec_cmd_process(V_GET_VPU_VERSION, (unsigned long *)CONV_PTR(&pInst->gsVpuDecVersion), pInst);
          if( ret != RETCODE_SUCCESS )
          {
    ...
    case VDEC_DEC_SEQ_HEADER:
    {
      ...
      if(pInst->gsVpuDecInit_Info.gsVpuDecInit.m_iFilePlayEnable != 0)
      {
        ...
        if( (pInst->extFunction & (TC_EXT_FUNC_MEM_PROTECTION_WITH_INSTREAM | TC_EXT_FUNC_MEM_PROTECTION_ONLY)) == 0x0u )
        {
          const unsigned char* ps = (const unsigned char*)pInst->gsBitstreamBufAddr[VA];
    ...
    {
      static char sname[] = "seqHeader_stream.bin";
      save_input_stream(sname, seq_stream_size, pInst, 0);
    }
    #endif
      ...
      {
      #if REMOVE_SECURE_COPY
        const vdec_info_t * pVdecInfo = (vdec_info_t *)CONV_PTR(&pInst->gsVpuDecInOut_Info.gsVpuDecInitialInfo);
      #else
        const vdec_info_t * pVdecInfo = (vdec_info_t *)CONV_PTR(&pInst->gsVpuDecSeqHeader_Info.gsVpuDecInitialInfo);
      #endif
        int max_width;
        int min_width;
        int min_height;

        max_width   = AVAILABLE_MAX_WIDTH;
    ...
    #endif
      LOGI( "[VDEC-%d] VDEC_DEC_SEQ_HEADER - Success mem_free = 0x%x ", pInst->vdec_instance_index, vdec_sys_final_free_mem(pInst) );
    ...
    case VDEC_DECODE:
    {
  ...
  #if defined(TC_SECURE_MEMORY_COPY)
      if( (pInst->extFunction & TC_EXT_FUNC_MEM_PROTECTION_WITH_INSTREAM) != 0x0u )
      {
      ...
      #ifdef VPU_IN_FRAME_DUMP
      {
        static char sname[] = "all_stream.bin"
        save_input_stream(sname, pInst->gsVpuDecInOut_Info.gsVpuDecInput.m_iBitstreamDataSize, pInst, 1);
      }
      #endif

      // Start decoding a frame.
      ret = vdec_cmd_process(V_DEC_DECODE, (unsigned long *)CONV_PTR(&pInst->gsVpuDecInOut_Info), pInst);
      ...
      )
      {
        ...
        LOGE("@@ Dec Out[%d] !! PicType[%d], OutIdx[%d/%d], OutStatus[%d/%d] ", ret,
        ...
        LOGE("DispOutIdx : %d ", pInst->gsVpuDecInOut_Info.gsVpuDecOutput.m_DecOutInfo.m_iDispOutIdx);

#ifdef VPU_IN_FRAME_DUMP
        if(ret == RETCODE_CODEC_EXIT) {
          static char sname[] = "error_codec_exit.bin";
          save_input_stream(sname, pInst->gsVpuDecInOut_Info.gsVpuDecInput.m_iBitstreamDataSize, pInst, 0);
        }
        else if(pInst->gsVpuDecInOut_Info.gsVpuDecOutput.m_DecOutInfo.m_iDecodingStatus == VPU_DEC_BUF_FULL){
          static char sname[] = "error_buffer_full.bin";
          save_input_stream(sname, pInst->gsVpuDecInOut_Info.gsVpuDecInput.m_iBitstreamDataSize, pInst, 0);
        }
        else {
          static char sname[] = "error_strange_res.bin";
          save_input_stream(sname, pInst->gsVpuDecInOut_Info.gsVpuDecInput.m_iBitstreamDataSize, pInst, 0);
        }
#endif
      }
      ...
      tcomx_memcpy(p_output_param, &pInst->gsVpuDecInOut_Info.gsVpuDecOutput, sizeof(dec_output_t ) );

      p_output_param->m_pInitialInfo = &pInst->gsCommDecInfo;

      if( (pInst->gsVpuDecInit_Info.gsVpuDecInit.m_iBitstreamFormat != STD_MJPG) || (pInst->gsUserInfo.m_bStillJpeg == 0u))
      {
  ...
  //    LOGE("Displayed addr 0x%x", p_output_param->m_pDispOut[PA][0]);
    ...
    case VDEC_GET_RING_BUFFER_STATUS:
    {
      ...
      ret = vdec_cmd_process(V_GET_RING_BUFFER_STATUS, (unsigned long *)CONV_PTR(&pInst->gsVpuDecBufStatus), pInst); // get the available space in the ring buffer
      if( ret != RETCODE_SUCCESS )
      {
    ...
    {
      vdec_ring_buffer_set_t* p_set_param = (vdec_ring_buffer_set_t*)CONV_PTR(pParam1);
      ...
      ret = vdec_cmd_process(V_FILL_RING_BUFFER_AUTO, (unsigned long *)CONV_PTR(&pInst->gsVpuDecBufFill), pInst);  // fille the Ring Buffer
    ...
    {
      pInst->gsVpuDecUpdateWP.iCopiedSize = (int)CONV_PTR(pParam1);
      pInst->gsVpuDecUpdateWP.iFlushBuf = (int)CONV_PTR(pParam2);

      ret = vdec_cmd_process(V_DEC_UPDATE_RINGBUF_WP, (unsigned long *)CONV_PTR(&pInst->gsVpuDecUpdateWP),  pInst);  // fille the Ring Buffer
  ...
  #endif
      ret = vdec_cmd_process(V_DEC_BUF_FLAG_CLEAR, (unsigned long *)CONV_PTR(&idx_display), pInst);
    ...
    {
      //vdec_input_t* p_input_param = (vdec_input_t*)CONV_PTR(pParam1);
      vdec_output_t* p_output_param = (vdec_output_t*)CONV_PTR(pParam2);
      ...
      ret = vdec_cmd_process(V_DEC_FLUSH_OUTPUT, (unsigned long *)CONV_PTR(&pInst->gsVpuDecInOut_Info), pInst);
    ...
    case VDEC_CLOSE:
    {
      ...
      {
        ret = vdec_cmd_process(V_DEC_CLOSE, (unsigned long *)CONV_PTR(&pInst->gsVpuDecInOut_Info), pInst);
        if( ret != RETCODE_SUCCESS )
        {
