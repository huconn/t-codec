...
static void hevc_copy_dec_out_info(hevc_dec_output_t *src, vdec_output_t *dst, tc_vdec_*  pInst)
{
...
static int hevc_cmd_process(int cmd, unsigned long* args, tc_vdec_ *pVdec)
{
  int ret;
  int success = 0;
  ...
  if((ret = ioctl(pInst->dec_fd, cmd, args)) < 0)
  {
    ...
    else
    {
      ...
      if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
        ...
        LOGE("HEVC[%d] ioctl err[%s] : cmd = 0x%x", pInst->vdec_instance_index, errmsg, cmd);
      }
    }
  }
  ...
  while (retry_cnt > 0) {
    ...
    if (ret < 0) {
      ...
      if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
        ...
        LOGE("HEVC[%d]-retry(%d:cmd(%d)) poll error '%s'", pInst->vdec_instance_index, retry_cnt, cmd, errmsg);
      }
      retry_cnt--;
      continue;
    }else if (ret == 0) {
  ...
  case V_DEC_INIT:
  {
    ...
    if(ioctl(pInst->dec_fd, V_DEC_INIT_RESULT, args) < 0){
      ...
      if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
        ...
        LOGE("HEVC[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_DEC_INIT_RESULT, errmsg);
      }
    }
  ...
  case V_DEC_SEQ_HEADER:
  {
    ...
    if(ioctl(pInst->dec_fd, V_DEC_SEQ_HEADER_RESULT, args) < 0){
      ...
      if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
        ...
        LOGE("HEVC[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_DEC_SEQ_HEADER_RESULT, errmsg);
      }
    }
    ret = seq_info->result;
  #ifdef ERROR_TEST
  ...
  case V_DEC_DECODE:
  {
    ...
    if(ioctl(pInst->dec_fd, V_DEC_DECODE_RESULT, args) < 0){
      ...
      if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
        ...
        LOGE("HEVC[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_DEC_DECODE_RESULT, errmsg);
      }
    }
  ...
  case V_DEC_FLUSH_OUTPUT:
  {
    ...
    if(ioctl(pInst->dec_fd, V_DEC_FLUSH_OUTPUT_RESULT, args) < 0){
      ...
      if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
        ...
        LOGE("HEVC[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_DEC_FLUSH_OUTPUT_RESULT, errmsg);
      }
    }
    ret = decoded_info->result;
  }
  ...
  case V_GET_RING_BUFFER_STATUS:
  {
    ...
    if(ioctl(pInst->dec_fd, V_GET_RING_BUFFER_STATUS_RESULT, args) < 0){
      ...
      if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
        ...
        LOGE("HEVC[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_GET_RING_BUFFER_STATUS_RESULT, errmsg);
      }
    }
    ret = p_param->result;
  }
  ...
  case V_FILL_RING_BUFFER_AUTO:
  {
    ...
    if(ioctl(pInst->dec_fd, V_FILL_RING_BUFFER_AUTO_RESULT, args) < 0){
      ...
      if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
        ...
        LOGE("HEVC[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_FILL_RING_BUFFER_AUTO_RESULT, errmsg);
      }
    }
    ret = p_param->result;
  }
  ...
  case V_DEC_UPDATE_RINGBUF_WP:
  {
    ...
    if(ioctl(pInst->dec_fd, V_DEC_UPDATE_RINGBUF_WP_RESULT, args) < 0){
      ...
      if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
        ...
        LOGE("HEVC[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_DEC_UPDATE_RINGBUF_WP_RESULT, errmsg);
      }
    }
    ret = p_param->result;
  }
  ...
  case V_GET_INITIAL_INFO_FOR_STREAMING_MODE_ONLY:
  {
    ...
    if(ioctl(pInst->dec_fd, V_GET_INITIAL_INFO_FOR_STREAMING_MODE_ONLY_RESULT, args) < 0){
      ...
      if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
        ...
        LOGE("HEVC[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_GET_INITIAL_INFO_FOR_STREAMING_MODE_ONLY_RESULT, errmsg);
      }
    }
    ret = p_param->result;
  }
  ...
  case V_GET_VPU_VERSION:
  {
    ...
    if(ioctl(pInst->dec_fd, V_GET_VPU_VERSION_RESULT, args) < 0){
      ...
      if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
        ...
        LOGE("HEVC[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_GET_VPU_VERSION_RESULT, errmsg);
      }
    }
    ret = p_param->result;
  }
  ...
  default:
    if(ioctl(pInst->dec_fd, V_DEC_GENERAL_RESULT, &ret) < 0){
      ...
      if (strerror_r(errno, errmsg, STRERR_BUFFERSIZE) != 0){
        ...
        LOGE("HEVC[%d] ioctl(0x%x) error[%s]!!", pInst->vdec_instance_index, V_DEC_GENERAL_RESULT, errmsg);
      }
    }
    break;
  }
...
static void print_hevc_initial_info( hevc_dec_initial_info_t* pInitialInfo, tc_vdec_ *pVdec)
{
...
int
hevc_dec_ready( hevc_dec_init_t* psHDecInit, tc_vdec_ *pVdec)
{
  tc_vdec_ * pInst = pVdec;
  ...
  pInst->gsBitstreamBufSize = vdec_sys_malloc_dual_addr(pInst->gsBitstreamBufAddr,
                              ((psHDecInit->m_iBitstreamBufSize > WAVE4_STREAM_BUF_SIZE) ? (pInst->gsBitstreamBufSize) : (WAVE4_STREAM_BUF_SIZE)),
                              ((psHDecInit->m_iBitstreamBufSize > WAVE4_STREAM_BUF_SIZE) ? (64*1024u) : (DPV_ALIGN_LEN)),
                              BUFFER_STREAM, pInst);

  if( (pInst->gsBitstreamBufAddr[PA] == 0u) || (pInst->gsBitstreamBufAddr[VA] == 0u)){
    DPRINTF( "[HEVC-%d] bitstream_buf_addr[%s] malloc() failed ", pInst->vdec_instance_index, (pInst->gsBitstreamBufAddr[PA] == 0U) ? ("PA") : ("VA"));
  ...
  if(pInst->gsHevcDecInit_Info.gsHevcDecInit.m_iFilePlayEnable == 0)
  {
    ...
    tcomx_zero_memset( (unsigned long*)pInst->gsIntermediateBufAddr[VA], 0x00 , pInst->gsIntermediateBufSize);
...
//  }

  {
    ...
    {
      pInst->gsUserdataBufSize = vdec_sys_malloc_dual_addr(pInst->gsUserdataBufAddr, WAVE4_USERDATA_BUF_SIZE, DPV_ALIGN_LEN, BUFFER_USERDATA, pInst );
      if( (pInst->gsUserdataBufAddr[PA] == 0) || (pInst->gsUserdataBufAddr[VA] == 0) )
      {
        DPRINTF( "[HEVC-%d:Err%d] pInst->gsUserdataBufAddr[%s] alloc failed ", pInst->vdec_instance_index, -1, (pInst->gsUserdataBufAddr[PA] == 0U) ? ("PA") : ("VA"));
        return -(VPU_NOT_ENOUGH_MEM);
      }
    ...
    pInst->gsBitWorkBufSize = ALIGN_BUFF(pInst->gsBitWorkBufSize, DPV_ALIGN_LEN);
    pInst->gsBitWorkBufAddr[PA] = (codec_addr_t)vdec_sys_malloc_physical_addr(&pInst->gsBitWorkBufAddr[K_VA], pInst->gsBitWorkBufSize, BUFFER_WORK, pInst );
    ...
    pInst->gsBitWorkBufAddr[VA] = (codec_addr_t)( (unsigned long *)pInst->gsBitWorkBufAddr[PA], pInst->gsBitWorkBufSize, pInst );
...
#else
    pInst->gsBitWorkBufSize = vdec_sys_malloc_dual_addr(pInst->gsBitWorkBufAddr, WAVE4_WORK_CODE_BUF_SIZE, DPV_ALIGN_LEN, BUFFER_WORK, pInst );
    if( (pInst->gsBitWorkBufAddr[PA] == 0) || (pInst->gsBitWorkBufAddr[VA] == 0) )
    {
      DPRINTF( "[HEVC-%d] bit_work_buf_addr[%s] alloc failed ", pInst->vdec_instance_index, (pInst->gsBitWorkBufAddr[PA] == 0U) ? ("PA") : ("VA"));
      return -(VPU_NOT_ENOUGH_MEM);
    }
    DSTATUS("[HEVC-%d] bit_work_buf_addr[PA] = 0x%lx, 0x%x ", pInst->vdec_instance_index, (codec_addr_t)pInst->gsBitWorkBufAddr[PA], pInst->gsBitWorkBufSize );
    DSTATUS("[HEVC-%d] bit_work_buf_addr[VA] = 0x%lx, 0x%x ", pInst->vdec_instance_index, (codec_addr_t)pInst->gsBitWorkBufAddr[VA], pInst->gsBitWorkBufSize );
...
int
hevc_dec_seq_header( int iSize, int iIsThumbnail, tc_vdec_ *pVdec )
{
  int ret = 0;
  tc_vdec_ * pInst = pVdec;
#if REMOVE_SECURE_COPY
  ...
  ret = hevc_cmd_process(V_DEC_SEQ_HEADER, (unsigned long *)CONV_PTR(&pInst->gsHevcDecInOut_Info), pInst);
  ...
  LOGD( "[HEVC-%d] FrameBufDelay %d, MinFrameBufferCount %d", pInst->vdec_instance_index, pInst->gsHevcDecInOut_Info.gsHevcDecInitialInfo.m_iFrameBufDelay , pInst->gsHevcDecSeqHeader_Info.gsHevcDecInitialInfo.m_iMinFrameBufferCount );
  {
    ...
    if(!iIsThumbnail){
#ifdef USE_MAP_CONVERTER
      if( (pInst->extFunction & TC_EXT_FUNC_USE_MAP_CONVERTER) != 0x0u )
      {
    ...
    max_count = vdec_sys_remain_memory_size(pInst) / pInst->gsHevcDecInOut_Info.gsHevcDecInitialInfo.m_iMinFrameBufferSize;
  ...
  pInst->gsFrameBufSize = ALIGN_BUFF( pInst->gsFrameBufSize, DPV_ALIGN_LEN );
  pInst->gsFrameBufAddr[PA] = (codec_addr_t)vdec_sys_malloc_physical_addr(&pInst->gsFrameBufAddr[K_VA], pInst->gsFrameBufSize, BUFFER_FRAMEBUFFER, pInst );
  ...
  pInst->gsFrameBufAddr[VA] = (codec_addr_t)vdec_sys_malloc_virtual_addr( (unsigned long *)pInst->gsFrameBufAddr[PA], pInst->gsFrameBufSize, pInst );
  ...
  ret = hevc_cmd_process(V_DEC_REG_FRAME_BUFFER, (unsigned long *)CONV_PTR(&pInst->gsHevcDecBuffer_Info), pInst);
  ...
  ret = hevc_cmd_process(V_DEC_SEQ_HEADER, (unsigned long *)CONV_PTR(&pInst->gsHevcDecSeqHeader_Info), pInst);
  if( ret != RETCODE_SUCCESS )
  {
           ...
           pInst->gsHevcDecSeqHeader_Info.gsHevcDecInitialInfo.m_iFrameBufDelay , pInst->gsHevcDecSeqHeader_Info.gsHevcDecInitialInfo.m_iMinFrameBufferCount );
  {
    ...
    max_count = vdec_sys_remain_memory_size(pInst) / pInst->gsHevcDecSeqHeader_Info.gsHevcDecInitialInfo.m_iMinFrameBufferSize;
  ...
  if ((pInst->gsFrameBufAddr[PA] == 0u) || (pInst->gsFrameBufAddr[VA] == 0u)){
    DPRINTF("[HEVC-%d] MinFrameBufferSize %d bytes, FrameBufSize %d bytes",
          pInst->vdec_instance_index, pInst->gsHevcDecSeqHeader_Info.gsHevcDecInitialInfo.m_iMinFrameBufferSize, pInst->gsFrameBufSize);
    DPRINTF("[HEVC-%d] frame_buf_addr[%s] malloc() failed",pInst->vdec_instance_index,(pInst->gsFrameBufAddr[PA] == 0U) ? ("PA") : ("VA"));
    return -(VPU_NOT_ENOUGH_MEM);
  }
...
static int hevc_set_user_data_info(hevc_dec_UserData_info_t* src, hevc_userdata_output_t *dst, tc_vdec_*  pInst) {
...
{
  int ret = 0;
  tc_vdec_ *pInst = (tc_vdec_ *)pParam3;
  (void)pHandle;
...
//  pInst->gsHevcDecInit_Info.gsHevcDecInit.m_bEnableVideoCache   = p_init_param->m_bEnableVideoCache;
  ...
  if( (pInst->extFunction & TC_EXT_FUNC_MAX_FRAMEBUFFER) != 0x0u )
  {
    pInst->gsHevcDecInit_Info.gsHevcDecInit.m_uiDecOptFlags |= (1u<<16);
    ...
    #if REMOVE_SECURE_COPY
    pInst->gsHevcDecInit_Info.gsHevcDecInit.m_uiDecOptFlags |= (1u<<26);
    #endif

#if HEVC_SUPPORT_SINGLE
    tcc_printf("\n\n[HEVC-%d] $ single core $\n\n", pInst->vdec_instance_index);
    pInst->gsHevcDecInit_Info.gsHevcDecInit.m_uiDecOptFlags |= (1u<<6);
#endif

#if HEVC_DISABLE_10BIT_OUT
    tcc_printf("\n\n[HEVC-%d] $ Disable 10bit output $\n\n", pInst->vdec_instance_index);
    pInst->gsHevcDecInit_Info.gsHevcDecInit.m_uiDecOptFlags |= (uint32_t)WAVE4_10BITS_DISABLE;
#endif
  ...
  if( (pInst->extFunction & TC_EXT_FUNC_NO_BUFFER_DELAY) != 0x0u )
  {
    ...
    pInst->gsHevcDecInit_Info.gsHevcDecInit.m_uiDecOptFlags |= (1u<<2);
  }
...
#endif
  {
    ...
    pInst->gsHevcDecInit_Info.gsHevcDecInit.m_uiDecOptFlags |= (uint32_t)WAVE4_WTL_ENABLE; // No compressed output!!, Must use 2 x framebuffer.
  }
  ...
  ret = hevc_cmd_process(V_DEC_INIT, (unsigned long *)CONV_PTR(&pInst->gsHevcDecInit_Info), pInst);
  if( ret != RETCODE_SUCCESS )
  {
  ...
  if( (pInst->extFunction & (TC_EXT_FUNC_MEM_PROTECTION_WITH_INSTREAM | TC_EXT_FUNC_MEM_PROTECTION_ONLY)) == 0x0u )
  {
    if (pInst->gsBitstreamBufSize > 100) {
      ...
      pInst->gsHevcDecVersion.pszVersion = &p_version[pInst->gsBitstreamBufSize - 100];
      pInst->gsHevcDecVersion.pszBuildData = &p_version[pInst->gsBitstreamBufSize - 50];

      ret = hevc_cmd_process(V_GET_VPU_VERSION, (unsigned long *)CONV_PTR(&pInst->gsHevcDecVersion), pInst);
      if( ret != RETCODE_SUCCESS )
      {
      ...
      else
      {
        ...
        LOGI( "[HEVC-%d] V_GET_HEVC_VERSION OK. Version is %.32s, and it's built at %.10s ", pInst->vdec_instance_index,
  ...
  //check the maximum/minimum video resolution limitation
  {
    ...
    max_width   = AVAILABLE_HEVC_MAX_WIDTH;
    max_height  = AVAILABLE_HEVC_MAX_HEIGHT;
...
#endif
  LOGI( "[HEVC-%d] VDEC_DEC_SEQ_HEADER - Success mem_free = 0x%x ", pInst->vdec_instance_index, vdec_sys_final_free_mem(pInst) );
  DSTATUS( "[HEVC] =======================================================" );
  }
  ...
  if( pInst->gsHevcDecInit_Info.gsHevcDecInit.m_iFilePlayEnable )
  {
    ...
    if( pInst->gsCurrStreamBuffer_index == 0 ){
      pInst->gsHevcDecInOut_Info.gsHevcDecInput.m_BitstreamDataAddr[PA] = pInst->gsBitstreamBufAddr[PA];
      pInst->gsHevcDecInOut_Info.gsHevcDecInput.m_BitstreamDataAddr[VA] = pInst->gsBitstreamBufAddr[K_VA];
      tcomx_memcpy( pInst->gsBitstreamBufAddr[VA], p_input_param->m_pInp[VA], pInst->gsHevcDecInOut_Info.gsHevcDecInput.m_iBitstreamDataSize);
    }
    else {
      ...
      tcomx_memcpy( (pInst->gsBitstreamBufAddr[VA] + pInst->gsBitstreamBufSize / 2), p_input_param->m_pInp[VA], pInst->gsHevcDecInOut_Info.gsHevcDecInput.m_iBitstreamDataSize);
    }
#else
    pInst->gsHevcDecInOut_Info.gsHevcDecInput.m_BitstreamDataAddr[PA] = pInst->gsBitstreamBufAddr[PA];
    pInst->gsHevcDecInOut_Info.gsHevcDecInput.m_BitstreamDataAddr[VA] = pInst->gsBitstreamBufAddr[K_VA];
  ...
  ret = hevc_cmd_process(V_DEC_DECODE, (unsigned long *)CONV_PTR(&pInst->gsHevcDecInOut_Info), pInst);
...
//    return ERR_END_OF_FILE;
  ...
  if(pInst->total_frm == 2)
  {
    ...
    vYaddr =(codec_addr_t)vdec_sys_malloc_virtual_addr(  (unsigned long *)p_output_param->m_MapConvInfo.m_CompressedY[0], p_output_param->m_pInitialInfo->m_iPicWidth *p_output_param->m_pInitialInfo->m_iPicHeight, pInst );
    vCBaddr =(codec_addr_t)vdec_sys_malloc_virtual_addr(  (unsigned long *)p_output_param->m_MapConvInfo.m_CompressedCb[0], p_output_param->m_pInitialInfo->m_iPicWidth *p_output_param->m_pInitialInfo->m_iPicHeight, pInst );
    vOffYaddr =(codec_addr_t)vdec_sys_malloc_virtual_addr(  (unsigned long *)p_output_param->m_MapConvInfo.m_FbcYOffsetAddr[0], p_output_param->m_pInitialInfo->m_iPicWidth *p_output_param->m_pInitialInfo->m_iPicHeight, pInst );
    vOffCBaddr =(codec_addr_t)vdec_sys_malloc_virtual_addr(  (unsigned long *)p_output_param->m_MapConvInfo.m_FbcCOffsetAddr[0], p_output_param->m_pInitialInfo->m_iPicWidth *p_output_param->m_pInitialInfo->m_iPicHeight, pInst );
    ...
    vdec_sys_free_virtual_addr(vYaddr,  p_output_param->m_pInitialInfo->m_iPicWidth *p_output_param->m_pInitialInfo->m_iPicHeight);
    vdec_sys_free_virtual_addr(vCBaddr,  p_output_param->m_pInitialInfo->m_iPicWidth *p_output_param->m_pInitialInfo->m_iPicHeight);
    vdec_sys_free_virtual_addr(vOffYaddr,  p_output_param->m_pInitialInfo->m_iPicWidth *p_output_param->m_pInitialInfo->m_iPicHeight);
    vdec_sys_free_virtual_addr(vOffCBaddr,  p_output_param->m_pInitialInfo->m_iPicWidth *p_output_param->m_pInitialInfo->m_iPicHeight);
  }
#endif//HEVC_OUT_FRAME_DUMP
...
//    LOGE("Displayed addr 0x%x", p_output_param->m_pDispOut[PA][0]);
  ...
  ret = hevc_cmd_process(V_GET_RING_BUFFER_STATUS, (unsigned long *)CONV_PTR(&pInst->gsHevcDecBufStatus), pInst); // get the available space in the ring buffer
  if( ret != RETCODE_SUCCESS )
  {
...
//    );
  ...
  else if( iOpCode == VDEC_FILL_RING_BUFFER )
  {
    ...
    ret = hevc_cmd_process(V_FILL_RING_BUFFER_AUTO, (unsigned long *)CONV_PTR(&pInst->gsHevcDecBufFill), pInst);  // fille the Ring Buffer
  ...
  ret = hevc_cmd_process(V_DEC_UPDATE_RINGBUF_WP, (unsigned long *)CONV_PTR(&pInst->gsHevcDecUpdateWP),  pInst);  // fille the Ring Buffer
  ...
  else if( iOpCode == VDEC_BUF_FLAG_CLEAR )
  {
    ...
    ret = hevc_cmd_process(V_DEC_BUF_FLAG_CLEAR, (unsigned long *)CONV_PTR(&idx_display), pInst);
...
//  pInst->gsHevcDecInOut_Info.gsHevcDecInput.m_iSkipFrameNum = 0;
  ...
  ret = hevc_cmd_process(V_DEC_FLUSH_OUTPUT, (unsigned long *)CONV_PTR(&pInst->gsHevcDecInOut_Info), pInst);
...
//    LOGE("Displayed addr 0x%x", p_output_param->m_pDispOut[PA][0]);
  ...
  {
    ret = hevc_cmd_process(V_DEC_CLOSE, (unsigned long *)CONV_PTR(&pInst->gsHevcDecInOut_Info), pInst);
    if( ret != RETCODE_SUCCESS )
    {
