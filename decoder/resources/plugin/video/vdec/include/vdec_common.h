...
#include "time.h"
uint32_t dec_time[30] = {0,};
uint32_t time_cnt = 0;
uint32_t total_dec_time = 0;
...
#define TC_EXT_FUNC_MEM_PROTECTION_WITH_INSTREAM (0x0001u)  // for read protection for VPU memory!! specially WFD-SINK!!
#define TC_EXT_FUNC_MEM_PROTECTION_ONLY					(0x0002u)  // for read protection for VPU memory!! specially WFD-SOURCE!!
#define TC_EXT_FUNC_NO_BUFFER_DELAY							(0x0004u)  // To ignore buffer-delay on VPU!!
#ifdef USE_MAP_CONVERTER
#define TC_EXT_FUNC_USE_MAP_CONVERTER						(0x0008u)  // To enable Map-Converter function for HEVC.
#endif
#define TC_EXT_FUNC_MAX_FRAMEBUFFER							(0x0010u)  // To configure reference frame buffer into 1080p for smooth-streamming(adaptive-streamming).
#define TC_EXT_FUNC_DISABLE_10BIT_OUTPUT					(0x0080u)  // disable 10 bit output (chip dependency, thumnail, etc.)
#define TC_EXT_FUNC_HIDE_SUPERFRAME_OUTPUT				(0x0100u)  // hide VP9 super-frame output to user-space
#define TC_EXT_FUNC_4KD2_USE_CQ_LEVEL_1					(0x0200u)  // To enable CQ Level 1 for VPU-4KD2
...
static int gs_iDbgmsgLevel_Vdec = 0;  //(0:None, 1:Error, 2:Warn, 3:Info, 4:Debug, 5:All)
...
#define DPV_ALIGN_LEN (4096u)

#ifndef ALIGN_BUFF
#define ALIGN_BUFF(buf, mul) (((uint32_t)(buf) + ((uint32_t)(mul)-1u)) & ~((uint32_t)(mul)-1u))
#endif

typedef int32_t (*DEC_FUNC)( int32_t iOpCode, unsigned long* pHandle, unsigned long* pParam1, unsigned long* pParam2 );

typedef struct tc_vdec_ {
...
void vpu_update_sizeinfo(int32_t format, uint32_t bps, uint32_t fps, int32_t image_width, int32_t image_height, unsigned long* pVdec);
...
extern cdk_func_t* gspfVDecList[VCODEC_MAX];
...
#endif
#ifdef CHANGE_INPUT_STREAM
void change_input_stream(unsigned char* out, int32_t* len, int32_t type, tc_vdec_ *pVdec);
#endif
...
float vdec_getAspectRatio(void *pvInstance, int iImgWidth, int iImgHeight);
