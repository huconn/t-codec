...
#define H264_MIN_OUTPUT_WIDTH				(DEFAULT_VIDEO_MIN_OUTPUT_WIDTH)
#define H264_MIN_OUTPUT_HEIGHT				(DEFAULT_VIDEO_MIN_OUTPUT_HEIGHT)
#define H264_MAX_OUTPUT_WIDTH				DEFAULT_VIDEO_MAX_OUTPUT_WIDTH //(1920)
#define H264_MAX_OUTPUT_HEIGHT				DEFAULT_VIDEO_MAX_OUTPUT_HEIGHT //(1088)
#define H264_DEFAULT_FRAMERATE				DEFAULT_VIDEO_DEFAULT_FRAMERATE //(30000)
#define H264_MAX_OUTPUT_REGION				(H264_MAX_OUTPUT_WIDTH * H264_MAX_OUTPUT_HEIGHT)
#define H264_MAX_FRAME_BUFFER_COUNT			(31)
...
typedef struct
{
	...
	unsigned char *pucSpsPpsBuffer;
} DEC_Plugin_H264_T;


static float decoder_plugin_h264_Get_AspectRatio(int iImgWidth, int iImgHeight, int iAspectRatio);
static int decoder_plugin_h264_AllocMemory(DEC_Plugin_H264_T *ptPlugin);
static int decoder_plugin_h264_FreeMemory(DEC_Plugin_H264_T *ptPlugin);
static int decoder_plugin_h264_DecodeSequenceHeader(DEC_Plugin_H264_T *ptPlugin, DP_Video_Config_SequenceHeader_T *ptSequenceHeader);

static DEC_PLUGIN_H decoder_plugin_h264_Open(void);
static int decoder_plugin_h264_Close(DEC_PLUGIN_H hPlugin);
static int decoder_plugin_h264_Init(DEC_PLUGIN_H hPlugin, void *pvInitConfig);
static int decoder_plugin_h264_Deinit(DEC_PLUGIN_H hPlugin);
static int decoder_plugin_h264_Decode(DEC_PLUGIN_H hPlugin, unsigned int uiStreamLen, unsigned char *pucStream, void *pvDecodingResult);
static int decoder_plugin_h264_Set(DEC_PLUGIN_H hPlugin, unsigned int uiId, void *pvStructure);
static int decoder_plugin_h264_Get(DEC_PLUGIN_H hPlugin, unsigned int uiId, void *pvStructure);


static float decoder_plugin_h264_Get_AspectRatio(int iImgWidth, int iImgHeight, int iAspectRatio)
{
	static const double AVC_Aspect_Ratio[] = {
		(0.0),        		// unspecified
		(1.0),        		// 1 : 1 Square
		(12.0  / 11.0),  	// 12:11
		(10.0  / 11.0),  	// 10:11
	...
	if ((iImgWidth >= 0) && (iImgHeight >= 0)) {
		unsigned int uiTmpAspectRatio = 0U;
		double dSAR = 0.0F;

		if (iAspectRatio >= 0) {
			uiTmpAspectRatio = (unsigned int)iAspectRatio;
		}

		if (((uiTmpAspectRatio >> 16U) & (unsigned int)0xFF) != 0U) {
			unsigned uiAspectRatioIdx = uiTmpAspectRatio >> 16U;

			if ((uiAspectRatioIdx > 1U) && (uiAspectRatioIdx < (sizeof(AVC_Aspect_Ratio) / sizeof(double)))) {
				dSAR = AVC_Aspect_Ratio[uiAspectRatioIdx];
				dDAR = ((double)iImgWidth * dSAR) / (double)iImgHeight;
			}
		} else {
			unsigned int uiSARWidth  = (uiTmpAspectRatio >> 16U) & (unsigned int)0xFFFF;
			unsigned int uiSARHeight = (uiTmpAspectRatio       ) & (unsigned int)0xFFFF;

			if ((uiSARWidth > 0U) && (uiSARHeight > 0U)) {
				dSAR = (double)uiSARWidth / (double)uiSARHeight;
			}

			if ((iImgWidth < INT16_MAX) && (iImgHeight < INT16_MAX)) {
				dDAR = ((double)iImgWidth * dSAR) / (double)iImgHeight;
			}
		}
	}
...
static int decoder_plugin_h264_AllocMemory(DEC_Plugin_H264_T *ptPlugin)
{
	...
	if (iBitstreamBufSize > 0) {
		int iSpsPpsSize = video_plugin_alloc_spspps_buffer(&ptPlugin->tDriverInfo, ptPlugin->pucSpsPpsBuffer);

		if (iSpsPpsSize >= 0) {
			ret = video_plugin_alloc_bitwork_buffer(&ptPlugin->tDriverInfo);
		} else {
			ret = -1;
		}
	} else {
...
static int decoder_plugin_h264_DecodeSequenceHeader(DEC_Plugin_H264_T *ptPlugin, DP_Video_Config_SequenceHeader_T *ptSequenceHeader)
{
	...
	if (iRetVal == 0) {
		int iSliceBufSize = video_plugin_alloc_slice_buffer(&ptPlugin->tDriverInfo);
		...
		if (iSliceBufSize >= 0) {
			...
			if (video_plugin_cmd(&ptPlugin->tDriverInfo, (VPC_DRIVER_TYPE_E)VPC_DRIVER_TYPE_VPU, VPC_COMMAND_VPU_GET_FREEMEM_SIZE, &iAvailBufSize) < 0) {
				...
				iRetVal = video_plugin_alloc_frame_buffer(&ptPlugin->tDriverInfo, iAvailBufSize, H264_MAX_FRAME_BUFFER_COUNT);
				...
				DPV_DETAIL_MSG("[%s(%d)]  - iSliceBufSize: %d, iFrameBufferSize: %d  \n", __func__, __LINE__, iSliceBufSize, ptPlugin->tDriverInfo.iFrameBufferSize);
	...
	if (iRetVal == 0) {
		VPC_RESOLUTION_INFO_T tMaxResInfo = {H264_MAX_OUTPUT_WIDTH, H264_MAX_OUTPUT_HEIGHT};
		VPC_RESOLUTION_INFO_T tMinResInfo = {H264_MIN_OUTPUT_WIDTH, H264_MIN_OUTPUT_HEIGHT};
...
static DEC_PLUGIN_H decoder_plugin_h264_Open(void)
{
	void *tmpPtr = DPV_CALLOC(sizeof(DEC_Plugin_H264_T));
	DEC_Plugin_H264_T *ptPlugin = NULL;
...
static int decoder_plugin_h264_Init(DEC_PLUGIN_H hPlugin, void *pvInitConfig)
{
	...
	ptPlugin->tDriverInfo.uiCodec 		= VPK_STD_H264;
	...
	if (ret != (DP_Video_Error_E)DP_Video_ErrorNone) {
		...
		ptDriverInfo->iBitstreamFormat 		= VPK_STD_H264;
		ptDriverInfo->iPicWidth				= H264_MAX_OUTPUT_WIDTH;
		ptDriverInfo->iPicHeight			= H264_MAX_OUTPUT_HEIGHT;
		ptDriverInfo->bEnableUserData		= 0;
		ptDriverInfo->bCbCrInterleaveMode 	= 1;
		ptDriverInfo->iFilePlayEnable 		= 1;
		ptDriverInfo->iFrameRate 			= H264_DEFAULT_FRAMERATE;
		ptDriverInfo->iBitRateMbps 			= 10;
		ptDriverInfo->iExtFunction 			= 0;

		video_plugin_set_init_info(ptDriverInfo);

		(void)decoder_plugin_h264_AllocMemory(ptPlugin);
...
static int decoder_plugin_h264_Deinit(DEC_PLUGIN_H hPlugin)
{
	...
	if (ptPlugin->bInited == false) {
		...
		(void)decoder_plugin_h264_FreeMemory(ptPlugin);
...
static int decoder_plugin_h264_Decode(DEC_PLUGIN_H hPlugin, unsigned int uiStreamLen, unsigned char *pucStream, void *pvDecodingResult)
{
	...
	if (uiStreamLen < (UINT32_MAX / 2U)) {
		...
		if (iDecResult == VPK_RETCODE_CODEC_EXIT) {
			...
			ptDecodingResult->fAspectRatio 	= decoder_plugin_h264_Get_AspectRatio(ptDecResult->tVideoResolution.iWidth, ptDecResult->tVideoResolution.iHeight, ptDecResult->iM2vAspectRatio);
		}
	} else {
		eRetErr = (DP_Video_Error_E)DP_Video_ErrorDecode;
	}
	...
	switch (eConfig) {
		...
		case DP_Video_Config_Set_VPU_Clock: {
			...
			if ((ptPlugin->tDriverInfo.iFrameRate > 0) && (ptPlugin->tDriverInfo.iBitRateMbps > 0)) {
				...
				tContentInfo.width 		= H264_MAX_OUTPUT_WIDTH;
				tContentInfo.height	 	= H264_MAX_OUTPUT_HEIGHT;
		...
		case DP_Video_Config_Set_SequenceHeader: {
			...
			iRetVal = decoder_plugin_h264_DecodeSequenceHeader(ptPlugin, ptSequenceHeader);
			break;
		}
...
DECODER_PLUGIN_DEFINE(
	DPV_H264,
	decoder_plugin_h264_Open,
	decoder_plugin_h264_Close,
	decoder_plugin_h264_Init,
	decoder_plugin_h264_Deinit,
	decoder_plugin_h264_Decode,
	decoder_plugin_h264_Set,
	decoder_plugin_h264_Get)