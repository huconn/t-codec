...
#define H265_MIN_OUTPUT_WIDTH				DEFAULT_VIDEO_MIN_OUTPUT_WIDTH
#define H265_MIN_OUTPUT_HEIGHT				DEFAULT_VIDEO_MIN_OUTPUT_HEIGHT
#if defined(SUPPORT_4K_VIDEO)
# define H265_MAX_OUTPUT_WIDTH   			DEFAULT_4K_VIDEO_MAX_OUTPUT_WIDTH
# define H265_MAX_OUTPUT_HEIGHT  			DEFAULT_4K_VIDEO_MAX_OUTPUT_HEIGHT
#else
# define H265_MAX_OUTPUT_WIDTH   			DEFAULT_VIDEO_MAX_OUTPUT_WIDTH
# define H265_MAX_OUTPUT_HEIGHT  			DEFAULT_VIDEO_MAX_OUTPUT_HEIGHT
#endif
#define H265_DEFAULT_FRAMERATE				DEFAULT_VIDEO_DEFAULT_FRAMERATE
#define H265_MAX_FRAME_BUFFER_COUNT			(31)
...
static int decoder_plugin_h265_AllocMemory(DEC_Plugin_H265_T *ptPlugin);
static int decoder_plugin_h265_FreeMemory(DEC_Plugin_H265_T *ptPlugin);
static int decoder_plugin_h265_DecodeSequenceHeader(DEC_Plugin_H265_T *ptPlugin, DP_Video_Config_SequenceHeader_T *ptSequenceHeader);

static DEC_PLUGIN_H decoder_plugin_h265_Open(void);
static int decoder_plugin_h265_Close(DEC_PLUGIN_H hPlugin);
static int decoder_plugin_h265_Init(DEC_PLUGIN_H hPlugin, void *pvInitConfig);
static int decoder_plugin_h265_Deinit(DEC_PLUGIN_H hPlugin);
static int decoder_plugin_h265_Decode(DEC_PLUGIN_H hPlugin, unsigned int uiStreamLen, unsigned char *pucStream, void *pvDecodingResult);
static int decoder_plugin_h265_Set(DEC_PLUGIN_H hPlugin, unsigned int uiId, void *pvStructure);
static int decoder_plugin_h265_Get(DEC_PLUGIN_H hPlugin, unsigned int uiId, void *pvStructure);
...
static int decoder_plugin_h265_DecodeSequenceHeader(DEC_Plugin_H265_T *ptPlugin, DP_Video_Config_SequenceHeader_T *ptSequenceHeader)
{
	...
	if (iRetVal == 0) {
		...
		if (iSliceBufSize >= 0) {
			...
			if (video_plugin_cmd(&ptPlugin->tDriverInfo, (VPC_DRIVER_TYPE_E)VPC_DRIVER_TYPE_VPU, VPC_COMMAND_VPU_GET_FREEMEM_SIZE, &iAvailBufSize) < 0) {
				...
				iRetVal = video_plugin_alloc_frame_buffer(&ptPlugin->tDriverInfo, iAvailBufSize, H265_MAX_FRAME_BUFFER_COUNT);
				...
				DPV_DETAIL_MSG("[%s(%d)]  - iSliceBufSize: %d, iBitstreamBufSize: %d(%d)   \n", __func__, __LINE__, iSliceBufSize, ptPlugin->tDriverInfo.iBitstreamBufSize, ptPlugin->tDriverInfo.iBitstreamFormat);
				DPV_DETAIL_MSG("[%s(%d)]  - iFrameBufferSize: %d(%d), m_iMinFrameBufferSize : %d \n", __func__, __LINE__, ptPlugin->tDriverInfo.iFrameBufferSize, ptPlugin->tDriverInfo.iFrameBufferCount, ptPlugin->tDriverInfo.iMinFrameBufferSize);
				DPV_DETAIL_MSG("[%s(%d)] ==================================================================== \n", __func__, __LINE__);
				#endif
			}
		} else {
	...
	if (iRetVal == 0) {
		VPC_RESOLUTION_INFO_T tMaxResInfo = {H265_MAX_OUTPUT_WIDTH, H265_MAX_OUTPUT_HEIGHT};
		VPC_RESOLUTION_INFO_T tMinResInfo = {H265_MIN_OUTPUT_WIDTH, H265_MIN_OUTPUT_HEIGHT};
...
static DEC_PLUGIN_H decoder_plugin_h265_Open(void)
{
	void *tmpPtr = DPV_CALLOC(sizeof(DEC_Plugin_H265_T));
	DEC_Plugin_H265_T *ptPlugin = NULL;
...
static int decoder_plugin_h265_Init(DEC_PLUGIN_H hPlugin, void *pvInitConfig)
{
	...
	ptPlugin->tDriverInfo.uiCodec 		= VPK_STD_H265;
	...
	ptPlugin->tDriverInfo.bEnable10Bits 		= false;
	ptPlugin->tDriverInfo.bEnableMapConverter 	= false;
	...
	if (ret != (DP_Video_Error_E)DP_Video_ErrorNone) {
		...
		ptDriverInfo->iBitstreamFormat 		= VPK_STD_H265;
		ptDriverInfo->iPicWidth				= H265_MAX_OUTPUT_WIDTH;
		ptDriverInfo->iPicHeight			= H265_MAX_OUTPUT_HEIGHT;
		ptDriverInfo->bEnableUserData		= 0;
		ptDriverInfo->bCbCrInterleaveMode 	= 1;
		ptDriverInfo->iFilePlayEnable 		= 1;
		ptDriverInfo->iFrameRate 			= H265_DEFAULT_FRAMERATE;
		ptDriverInfo->iBitRateMbps 			= 10;
		ptDriverInfo->iExtFunction 			= 0;

		video_plugin_set_init_info(ptDriverInfo);

		(void)decoder_plugin_h265_AllocMemory(ptPlugin);
		(void)video_plugin_poll_cmd(&ptPlugin->tDriverInfo, (VPC_DRIVER_TYPE_E)VPC_DRIVER_TYPE_VPU, VPC_COMMAND_V_DEC_INIT, NULL);

		video_plugin_set_frame_skip_mode(&ptPlugin->tDriverInfo, VPC_FRAME_SKIP_MODE_I_SEARCH);
...
static int decoder_plugin_h265_Deinit(DEC_PLUGIN_H hPlugin)
{
	...
	if (ptPlugin->bInited == false) {
		...
		(void)decoder_plugin_h265_FreeMemory(ptPlugin);
	...
	switch (eConfig) {
		...
		case DP_Video_Config_Set_VPU_Clock: {
			...
			if ((ptPlugin->tDriverInfo.iFrameRate > 0) && (ptPlugin->tDriverInfo.iBitRateMbps > 0)) {
				...
				tContentInfo.width 		= H265_MAX_OUTPUT_WIDTH;
				tContentInfo.height	 	= H265_MAX_OUTPUT_HEIGHT;
		...
		case DP_Video_Config_Set_SequenceHeader: {
			...
			iRetVal = decoder_plugin_h265_DecodeSequenceHeader(ptPlugin, ptSequenceHeader);
			break;
		}
...
DECODER_PLUGIN_DEFINE(
	DPV_H265,
	decoder_plugin_h265_Open,
	decoder_plugin_h265_Close,
	decoder_plugin_h265_Init,
	decoder_plugin_h265_Deinit,
	decoder_plugin_h265_Decode,
	decoder_plugin_h265_Set,
	decoder_plugin_h265_Get)