...
#define MPG2_MIN_OUTPUT_WIDTH				(DEFAULT_VIDEO_MIN_OUTPUT_WIDTH)
#define MPG2_MIN_OUTPUT_HEIGHT				(DEFAULT_VIDEO_MIN_OUTPUT_HEIGHT)
#define MPG2_MAX_OUTPUT_WIDTH				DEFAULT_VIDEO_MAX_OUTPUT_WIDTH //(1920)
#define MPG2_MAX_OUTPUT_HEIGHT				DEFAULT_VIDEO_MAX_OUTPUT_HEIGHT //(1088)
#define MPG2_DEFAULT_FRAMERATE				DEFAULT_VIDEO_DEFAULT_FRAMERATE //(30000)
#define MPG2_MAX_OUTPUT_REGION				(MPG2_MAX_OUTPUT_WIDTH * MPG2_MAX_OUTPUT_HEIGHT)
#define MPG2_MAX_FRAME_BUFFER_COUNT			(31)
...
static int decoder_plugin_mpeg2_AllocMemory(DEC_Plugin_MPG2_T *ptPlugin);
static int decoder_plugin_mpeg2_FreeMemory(DEC_Plugin_MPG2_T *ptPlugin);
static int decoder_plugin_mpeg2_DecodeSequenceHeader(DEC_Plugin_MPG2_T *ptPlugin, DP_Video_Config_SequenceHeader_T *ptSequenceHeader);

static DEC_PLUGIN_H decoder_plugin_mpeg2_Open(void);
static int decoder_plugin_mpeg2_Close(DEC_PLUGIN_H hPlugin);
static int decoder_plugin_mpeg2_Init(DEC_PLUGIN_H hPlugin, void *pvInitConfig);
static int decoder_plugin_mpeg2_Deinit(DEC_PLUGIN_H hPlugin);
static int decoder_plugin_mpeg2_Decode(DEC_PLUGIN_H hPlugin, unsigned int uiStreamLen, unsigned char *pucStream, void *pvDecodingResult);
static int decoder_plugin_mpeg2_Set(DEC_PLUGIN_H hPlugin, unsigned int uiId, void *pvStructure);
static int decoder_plugin_mpeg2_Get(DEC_PLUGIN_H hPlugin, unsigned int uiId, void *pvStructure);
...
static int decoder_plugin_mpeg2_DecodeSequenceHeader(DEC_Plugin_MPG2_T *ptPlugin, DP_Video_Config_SequenceHeader_T *ptSequenceHeader)
{
	...
	if (iRetVal == 0) {
		...
		if (iSliceBufSize >= 0) {
			...
			if (video_plugin_cmd(&ptPlugin->tDriverInfo, (VPC_DRIVER_TYPE_E)VPC_DRIVER_TYPE_VPU, VPC_COMMAND_VPU_GET_FREEMEM_SIZE, &iAvailBufSize) < 0) {
				...
				iRetVal = video_plugin_alloc_frame_buffer(&ptPlugin->tDriverInfo, iAvailBufSize, MPG2_MAX_FRAME_BUFFER_COUNT);
	...
	if (iRetVal == 0) {
		VPC_RESOLUTION_INFO_T tMaxResInfo = {MPG2_MAX_OUTPUT_WIDTH, MPG2_MAX_OUTPUT_HEIGHT};
		VPC_RESOLUTION_INFO_T tMinResInfo = {MPG2_MIN_OUTPUT_WIDTH, MPG2_MIN_OUTPUT_HEIGHT};
...
static DEC_PLUGIN_H decoder_plugin_mpeg2_Open(void)
{
	void *tmpPtr = DPV_CALLOC(sizeof(DEC_Plugin_MPG2_T));
	DEC_Plugin_MPG2_T *ptPlugin = NULL;
...
static int decoder_plugin_mpeg2_Init(DEC_PLUGIN_H hPlugin, void *pvInitConfig)
{
	...
	ptPlugin->tDriverInfo.uiCodec 		= VPK_STD_MPEG2;
	...
	if (ret != (DP_Video_Error_E)DP_Video_ErrorNone) {
		...
		ptDriverInfo->iBitstreamFormat 		= VPK_STD_MPEG2;
		ptDriverInfo->iPicWidth				= MPG2_MAX_OUTPUT_WIDTH;
		ptDriverInfo->iPicHeight			= MPG2_MAX_OUTPUT_HEIGHT;
		ptDriverInfo->bEnableUserData		= 0;
		ptDriverInfo->bCbCrInterleaveMode 	= 1;
		ptDriverInfo->iFilePlayEnable 		= 1;
		ptDriverInfo->iFrameRate 			= MPG2_DEFAULT_FRAMERATE;
		ptDriverInfo->iBitRateMbps 			= 10;
		ptDriverInfo->iExtFunction 			= 0;

		video_plugin_set_init_info(ptDriverInfo);

		(void)decoder_plugin_mpeg2_AllocMemory(ptPlugin);
...
static int decoder_plugin_mpeg2_Deinit(DEC_PLUGIN_H hPlugin)
{
	...
	if (ptPlugin->bInited == false) {
		...
		(void)decoder_plugin_mpeg2_FreeMemory(ptPlugin);
	...
	switch (eConfig) {
		...
		case DP_Video_Config_Set_VPU_Clock: {
			...
			if ((ptPlugin->tDriverInfo.iFrameRate > 0) && (ptPlugin->tDriverInfo.iBitRateMbps > 0)) {
				...
				tContentInfo.width 		= MPG2_MAX_OUTPUT_WIDTH;
				tContentInfo.height	 	= MPG2_MAX_OUTPUT_HEIGHT;
		...
		case DP_Video_Config_Set_SequenceHeader: {
			...
			iRetVal = decoder_plugin_mpeg2_DecodeSequenceHeader(ptPlugin, ptSequenceHeader);
			break;
		}
...
DECODER_PLUGIN_DEFINE(
	DPV_MPG2,
	decoder_plugin_mpeg2_Open,
	decoder_plugin_mpeg2_Close,
	decoder_plugin_mpeg2_Init,
	decoder_plugin_mpeg2_Deinit,
	decoder_plugin_mpeg2_Decode,
	decoder_plugin_mpeg2_Set,
	decoder_plugin_mpeg2_Get)