...
#define DUMMY_MIN_OUTPUT_WIDTH				DEFAULT_VIDEO_MIN_OUTPUT_WIDTH
#define DUMMY_MIN_OUTPUT_HEIGHT				DEFAULT_VIDEO_MIN_OUTPUT_HEIGHT
#define DUMMY_MAX_OUTPUT_WIDTH				DEFAULT_VIDEO_MAX_OUTPUT_WIDTH
#define DUMMY_MAX_OUTPUT_HEIGHT				DEFAULT_VIDEO_MAX_OUTPUT_HEIGHT
#define DUMMY_DEFAULT_FRAMERATE				DEFAULT_VIDEO_DEFAULT_FRAMERATE

#define DUMMY_MAX_FRAME_BUFFER_COUNT		(31)
...
static int decoder_plugin_dummy_AllocMemory(DEC_Plugin_DUMMY_T *ptPlugin);
static int decoder_plugin_dummy_FreeMemory(DEC_Plugin_DUMMY_T *ptPlugin);
static int decoder_plugin_dummy_DecodeSequenceHeader(DEC_Plugin_DUMMY_T *ptPlugin, DP_Video_Config_SequenceHeader_T *ptSequenceHeader);

static DEC_PLUGIN_H decoder_plugin_dummy_Open(void);
static int decoder_plugin_dummy_Close(DEC_PLUGIN_H hPlugin);
static int decoder_plugin_dummy_Init(DEC_PLUGIN_H hPlugin, void *pvInitConfig);
static int decoder_plugin_dummy_Deinit(DEC_PLUGIN_H hPlugin);
static int decoder_plugin_dummy_Decode(DEC_PLUGIN_H hPlugin, unsigned int uiStreamLen, unsigned char *pucStream, void *pvDecodingResult);
static int decoder_plugin_dummy_Set(DEC_PLUGIN_H hPlugin, unsigned int uiId, void *pvStructure);
static int decoder_plugin_dummy_Get(DEC_PLUGIN_H hPlugin, unsigned int uiId, void *pvStructure);
...
static int decoder_plugin_dummy_DecodeSequenceHeader(DEC_Plugin_DUMMY_T *ptPlugin, DP_Video_Config_SequenceHeader_T *ptSequenceHeader)
{
	...
	if (iRetVal == 0) {
		...
		if (iSliceBufSize >= 0) {
			...
			if (video_plugin_cmd(&ptPlugin->tDriverInfo, (VPC_DRIVER_TYPE_E)VPC_DRIVER_TYPE_VPU, VPC_COMMAND_VPU_GET_FREEMEM_SIZE, &iAvailBufSize) < 0) {
				...
				iRetVal = video_plugin_alloc_frame_buffer(&ptPlugin->tDriverInfo, iAvailBufSize, DUMMY_MAX_FRAME_BUFFER_COUNT);
	...
	if (iRetVal == 0) {
		VPC_RESOLUTION_INFO_T tMaxResInfo = {DUMMY_MAX_OUTPUT_WIDTH, DUMMY_MAX_OUTPUT_HEIGHT};
		VPC_RESOLUTION_INFO_T tMinResInfo = {DUMMY_MIN_OUTPUT_WIDTH, DUMMY_MIN_OUTPUT_HEIGHT};
...
static DEC_PLUGIN_H decoder_plugin_dummy_Open(void)
{
	void *tmpPtr = DPV_CALLOC(sizeof(DEC_Plugin_DUMMY_T));
	DEC_Plugin_DUMMY_T *ptPlugin = NULL;
...
static int decoder_plugin_dummy_Deinit(DEC_PLUGIN_H hPlugin)
{
	...
	if (ptPlugin->bInited == false) {
		...
		(void)decoder_plugin_dummy_FreeMemory(ptPlugin);
	...
	switch (eConfig) {
		...
		case DP_Video_Config_Set_VPU_Clock: {
			...
			if ((ptPlugin->tDriverInfo.iFrameRate > 0) && (ptPlugin->tDriverInfo.iBitRateMbps > 0)) {
				...
				tContentInfo.width 		= DUMMY_MAX_OUTPUT_WIDTH;
				tContentInfo.height	 	= DUMMY_MAX_OUTPUT_HEIGHT;
		...
		case DP_Video_Config_Set_SequenceHeader: {
			...
			iRetVal = decoder_plugin_dummy_DecodeSequenceHeader(ptPlugin, ptSequenceHeader);
			break;
		}
...
DECODER_PLUGIN_DEFINE(
	"dummy",
	decoder_plugin_dummy_Open,
	decoder_plugin_dummy_Close,
	decoder_plugin_dummy_Init,
	decoder_plugin_dummy_Deinit,
	decoder_plugin_dummy_Decode,
	decoder_plugin_dummy_Set,
	decoder_plugin_dummy_Get)