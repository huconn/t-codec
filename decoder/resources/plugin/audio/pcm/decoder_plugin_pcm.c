...
#define CODEC_PCM_DEC_LIB_NAME	"libtccpcmdec.so"
...
static int SF_PCM_LoadLibrary(DEC_Plugin_PCM_T *ptPlugin, const char *pstrLibName)
{
	...
	if(ptPlugin->hLib != NULL)
	{
		...
		if(ptCodecFunc == NULL)
		{
			audio_printf("[PCM] dlsym failed! - %s\n", pstrLibName);
	...
	{
		audio_printf("[PCM] dlopen failed! - %s: %s\n", pstrLibName, dlerror());
		ret = -1;
	}
...
static void SF_PCM_UnloadLibrary(DEC_Plugin_PCM_T *ptPlugin)
{
	if(ptPlugin->hLib != NULL)
	{
		(void)dlclose(ptPlugin->hLib);
...
static DEC_PLUGIN_H decoder_plugin_pcm_Open(void)
{
	int iVal;
	...
	DEC_Plugin_PCM_T *ptPlugin = (DEC_Plugin_PCM_T *)DecPlugin_CastAddr_VOID_UINTPTR(g_malloc(sizeof(DEC_Plugin_PCM_T)));
	if (ptPlugin != NULL)
	{
		(void)memset(ptPlugin, 0, sizeof(DEC_Plugin_PCM_T));

		iVal = SF_PCM_LoadLibrary(ptPlugin, CODEC_PCM_DEC_LIB_NAME);
...
static int decoder_plugin_pcm_Close(DEC_PLUGIN_H hPlugin)
{
	DEC_Plugin_PCM_T *ptPlugin = (DEC_Plugin_PCM_T *)DecPlugin_CastAddr_VOID_UINTPTR(hPlugin);
	if(ptPlugin != NULL)
	{
		SF_PCM_UnloadLibrary(ptPlugin);
		audio_free(ptPlugin);
	}
	return 0;
}

static int decoder_plugin_pcm_Init(DEC_PLUGIN_H hPlugin)
{
	int iRetVal = 0;
	...
	ST_PCM_DecoderPrivate_T *ptDecoderPrivate;
	...
	ptDecoderPrivate = (ST_PCM_DecoderPrivate_T *)DecPlugin_CastAddr_VOID_UINTPTR(ptDecInit->m_unAudioCodecParams);
	...
	ptDecoderPrivate->m_iWAVForm_TagID = 1;
	ptDecoderPrivate->m_uiNBlockAlign = 1024;
	ptDecoderPrivate->m_iEndian = 0;
	ptDecoderPrivate->m_iContainerType = 0;
...
static int decoder_plugin_pcm_Deinit(DEC_PLUGIN_H hPlugin)
{
	int iRetVal;
...
static int decoder_plugin_pcm_Decode(DEC_PLUGIN_H hPlugin, unsigned int uiStreamLen, const unsigned char *pucStream, void *pvDecodedStream)
{
...
static int decoder_plugin_pcm_Set(DEC_PLUGIN_H hPlugin, unsigned int uiId, void *pvStructure)
{
	int iRetVal = 0;
	...
	case DP_Audio_Config_Descrioptor:
		{
			...
			audio_printf("[PCM_SET] Descriptor Info[Stream: %d %d %d Pcm:%d CodecData:%p %d]\n",
			...
			ptDecoderPrivate->m_iWAVForm_TagID = ptDescriptor->pcm.iWAVForm_TagID;
			ptDecoderPrivate->m_uiNBlockAlign = ptDescriptor->pcm.uiNBlockAlign;
			ptDecoderPrivate->m_iEndian = ptDescriptor->pcm.iEndian;
			ptDecoderPrivate->m_iContainerType = ptDescriptor->pcm.iContainerType;
		}
		break;
	default:
		{
...
static int decoder_plugin_pcm_Get(DEC_PLUGIN_H hPlugin, unsigned int uiId, void *pvStructure)
{
	int iRetVal = 0;

	const DEC_Plugin_PCM_T *ptPlugin = (const DEC_Plugin_PCM_T *)DecPlugin_CastAddr_VOID_UINTPTR(hPlugin);
...
DECODER_PLUGIN_DEFINE(
	"pcm",
	decoder_plugin_pcm_Open,
	decoder_plugin_pcm_Close,
	decoder_plugin_pcm_Init,
	decoder_plugin_pcm_Deinit,
	decoder_plugin_pcm_Decode,
	decoder_plugin_pcm_Set,
	decoder_plugin_pcm_Get)
